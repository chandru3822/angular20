...
export function createDBChangeLogQuery(database, db_id) {
  const db_query_data = {
    ahjDesign: {
      custom_field_audit_table: 'feat_db_ahj_design_custom_field_value_audit',
      custom_field_value_table: 'feat_db_ahj_design_custom_field_value',
      database_table: 'feat_db_ahj',
      database_sub_table: 'feat_db_ahj_design',
      database_sub_table_id_variable_name: 'ahj_design_id',
      database_table_id_variable_name: 'ahj_id',
      custom_field_value_to_match: 'ahj_design_custom_field_value_id',
    },
    ahjInspection: {
      custom_field_audit_table: 'feat_db_ahj_inspection_custom_field_value_audit',
      custom_field_value_table: 'feat_db_ahj_inspection_custom_field_value',
      database_table: 'feat_db_ahj',
      database_sub_table: 'feat_db_ahj_inspection',
      database_sub_table_id_variable_name: 'ahj_inspection_id',
      database_table_id_variable_name: 'ahj_id',
      custom_field_value_to_match: 'ahj_inspection_custom_field_value_id',
    },
    ahjPermit: {
      custom_field_audit_table: 'feat_db_ahj_permit_custom_field_value_audit',
      custom_field_value_table: 'feat_db_ahj_permit_custom_field_value',
      database_table: 'feat_db_ahj',
      database_sub_table: 'feat_db_ahj_permit',
      database_sub_table_id_variable_name: 'ahj_permit_id',
      database_table_id_variable_name: 'ahj_id',
      custom_field_value_to_match: 'ahj_permit_custom_field_value_id',
    },
    utility: {
      custom_field_audit_table: 'feat_db_utility_custom_field_value_audit',
      custom_field_value_table: 'feat_db_utility_custom_field_value',
      database_table: 'feat_db_utility',
      database_table_id_variable_name: 'feat_db_utility_id',
      database_sub_table: '',
      database_sub_table_id_variable_name: '',
      custom_field_value_to_match: 'feat_db_utility_custom_field_value_id',
    },
    hoa: {
      custom_field_audit_table: 'feat_db_hoa_custom_field_value_audit',
      custom_field_value_table: 'feat_db_hoa_custom_field_value',
      database_table: 'feat_db_hoa',
      database_table_id_variable_name: 'hoa_id',
      database_sub_table: '',
      database_sub_table_id_variable_name: '',
      custom_field_value_to_match: 'hoa_custom_field_value_id',
    },
    supplier: {
      custom_field_audit_table: 'feat_db_supplier_custom_field_value_audit',
      custom_field_value_table: 'feat_db_supplier_custom_field_value',
      database_table: 'feat_db_supplier',
      database_table_id_variable_name: 'feat_db_supplier_id',
      database_sub_table: '',
      database_sub_table_id_variable_name: '',
      custom_field_value_to_match: 'feat_db_supplier_custom_field_value_id',
    },
    incentive: {
      custom_field_audit_table: 'feat_db_incentive_custom_field_value_audit',
      custom_field_value_table: 'feat_db_incentive_custom_field_value',
      database_table: 'feat_db_incentive',
      database_table_id_variable_name: 'feat_db_incentive_id',
      database_sub_table: '',
      database_sub_table_id_variable_name: '',
      custom_field_value_to_match: 'feat_db_incentive_custom_field_value_id',
    },
  ...
  var query = `
    select
      ...
      cf.field_name,
      case
        when dt.id = 6 and cdt.has_list_values is false then cfva.old_value
        when dt.id = 6 and cdt.has_list_values is true then (
          select lov.name from brs.list_of_value lov where lov.id = cfva.old_value::bigint)::text
      when dt.id = 5 then cfva.old_value
      when dt.id = 4 then cfva.old_value::text
    end as previous_value,
    case
      when dt.id = 6 and cdt.has_list_values is false then cfva.new_value
      when dt.id = 6 and cdt.has_list_values is true then (
    select lov.name from brs.list_of_value lov where lov.id = cfva.new_value::bigint)::text
      when dt.id = 5 then cfva.new_value
      when dt.id = 4 then cfva.new_value::text
    end as updated_value,
        cfva.date_modified,
        concat(u.first_name, ' ', u.last_name) as modified_by
    from brs.${db_query_data[database].custom_field_audit_table} cfva
    join brs.${db_query_data[database].custom_field_value_table} cfv on cfv.id = cfva.${db_query_data[database].custom_field_value_to_match}`

  if (db_query_data[database].database_sub_table !== '') {
    query = query.concat('\n', `\tjoin brs.${db_query_data[database].database_sub_table} db_sub_tbl on cfv.${db_query_data[database].database_sub_table_id_variable_name} = db_sub_tbl.id`)
    query = query.concat('\n', `\tjoin brs.${db_query_data[database].database_table} db_tbl on db_sub_tbl.${db_query_data[database].database_table_id_variable_name} = db_tbl.id`)
  } else {
    query = query.concat('\n', `\tjoin brs.${db_query_data[database].database_table} db_tbl on cfv.${db_query_data[database].database_table_id_variable_name} = db_tbl.id`)
  }

  let the_rest = `\tjoin brs.custom_field_group_assignment cfga on cfv.custom_field_group_assignment_id = cfga.id
    ...
    join flow.\"user\" u on cfva.modified_by_id= u.id
    where ${db_query_data[database].database_table_id_variable_name} = ${db_id}
    order by cfva.date_modified desc;
  `
