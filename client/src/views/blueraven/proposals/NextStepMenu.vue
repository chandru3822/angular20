...
const creditStatus = ref(undefined)
...
const isCreditCheckRequired = computed(() => {
  ...
  const cfv = proposal.value.customFieldGroups.find(x => x.id === 27)?.customFieldValues?.find(x => x.customFieldId === 128)
...
const isEmailValid = computed(() => {
  ...
  return confirmEmail.value.emailAddress.trim().length > 0 && /.+@.+[.].+/.test(confirmEmail.value.emailAddress)
})
...
const submitCreditCheck = async() => {
  ...
  if (!proposal.value.creditCheckSubmitted) {
    ...
    if (value.email?.trim().length > 0 && value.updated) {
      ...
      try {
        await putRequest(`/install-agreement/updateEmailAddress/${proposal.value.projectId}`, body, 'blueraven')
      } catch (e) {
  ...
  try {
    const {status, data} = await getRequest(`/proposal/${proposal.value.id}/loanApplication`, 'blueraven')
    if (status !== 200) {
...
const lockProposal = async() => {
  // console.log("required", requiredFields.value)
  // return false
  ...
  try {
    ...
    const {data} = await postRequest(`/proposal/${proposal.value.id}/lock`, {}, 'blueraven')
...
const sendDocs = async(docType) => {
  ...
  try {
    appStore.loading = true
    const {data = {}} = await postRequest(`/proposal/${proposal.value.id}/sendDocs`,
        {
          docType,
