...
onMounted(() => {
  Promise.allSettled([getProposalDetail(props.id), getProposalObjectTypes()])
})
...
const archiveItem = async (item) => {
  ...
  try {
    ...
    await postRequest(
      `/proposal/versions/${props.id}/values/${propType.value.code}/${item.pk}/archive`,
      undefined,
      'blueraven'
    )
...
const deleteItem = async (item) => {
  const { data } = await deleteRequestWithPayload(
    `/proposal/versions/${props.id}/values/${propType.value.code}/${item.pk}`,
    'blueraven'
  )
...
const undoAllChanges = async () => {
  const { data } = await postRequest(
    `/proposal/versions/${props.id}/values/${propType.value.code}/reset`,
    {},
    'blueraven'
  )
  ...
  appStore.showSnack(
    'SUCCESS',
    `All changes to "${propType.value.name}" successfully reverted!`
  )
}
const getProposalDetail = async (proposalVersionId) => {
  try {
    const { data } = await getRequestWithParams(
      `/proposal/versions/${proposalVersionId}`,
      {},
      'blueraven'
    )
...
const getProposalObjectTypeFields = async (objectType) => {
  const { data } = await getRequestWithParams(
    `/proposal/versions/fields/${objectType}`,
    {},
    'blueraven'
  )
...
const getProposalObjectTypeFieldValues = async (
  proposalVersionId,
  objectType
) => {
  const { data } = await getRequestWithParams(
    `/proposal/versions/${proposalVersionId}/values/${objectType}`,
    {},
    'blueraven'
  )
...
const publish = async (proposalVersionId, message) => {
  try {
    const { data } = await postRequest(
      `/proposal/versions/${proposalVersionId}/publish`,
      { message },
      'blueraven'
    )
    detail.value = { ...data }
    ...
    appStore.showSnack(
      'SUCCESS',
      `Proposal Version #${proposalVersionId} Successfully Published`
    )
  } catch (e) {
...
const doSaveValues = async (group) => {
  try {
    const { data = {} } = await postRequest(
      `/proposal/versions/${detail.value.id}/values/${propType.value.code}`,
      group,
      'blueraven'
    )
