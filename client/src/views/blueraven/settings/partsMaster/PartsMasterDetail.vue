  ...
  <div>
    ...
    <v-toolbar flat class="app-toolbar">
      <v-toolbar-title v-if="detail">
        ...
        <v-chip class="ma-2" color="grey lighten-2" label>
    ...
    <div v-if="detail">
      <div class="top-actions">
        ...
        <fragment v-if="isDraft">
          ...
          ></a-btn>
          <ConfirmationDialog
            :open-dialog="undoDraftChanges"
            @confirm="undoAllChanges"
            @close-dialog="undoDraftChanges = false"
          >
            <template v-slot:title>Confirm</template>
            <p>
              Are you sure you want to undo all changes to this draft?
              <span class="error--text">WARNING:</span>This action is
              <b>irreversible</b>!
            </p>
          ...
          <a-btn
            ...
            text="Add New"
          ></a-btn>
          <NewPartsMasterValueDialog
            v-if="partType"
            :visible="visible"
            :editing="editedItem"
            :object-code="partType.code"
            @input="doInput"
            @save="doSaveValues"
          />
        </fragment>
      ...
      <ConfirmationDialog
        :open-dialog="!!selectedDeleteItem"
        ...
        >
              ...
              <v-row no-gutters>
                ...
                <v-col cols="4" v-if="isDraft">
                  <v-switch
                    ...
                    inset
                    :label="`${
                      !modifiedOnlyFilter ? 'Show Modified Only' : 'Show All'
                    }`"
                  ></v-switch>
                </v-col>
              </v-row>
          ...
          <template #item="{ item, headers }">
            <tr
              ...
              :title="item.archived ? 'This row has been archived' : ''"
            ...
            >
              <td v-for="header in headers">
                ...
                <span v-if="item[header.value]">
                  {{ customValueFormatter(item[header.value]) }}
                </span>
              </td>
            </tr>
          </template>
        </v-data-table>
        <p v-else>No type selected</p>
      </div>
    </div>
    <div v-else>
      <p>This is not the parts master version you are looking for...</p>
    </div>
  </div>
</template>
<script setup>
...
import NewPartsMasterValueDialog from './NewPartsMasterValueDialog.vue'
...
import { capitalize, customValueFormatter } from './formatters.js'
...
onMounted(() => {
  Promise.allSettled([getPartsMasterDetail(props.id), getPartsMasterObjectTypes()])
})

const search = ref('')
const options = ref({})
const detail = ref({})
const partType = ref(undefined)
const headers = ref([])
const values = ref([])
const types = ref([])
...
const rows = computed(() => {
  //intentional '=='
  return !modifiedOnlyFilter.value
    ? values.value
    : values.value?.filter((v) => v.versionId === props.id)
})
...
const changer = async () => {
  const requests = []
  const { code } = partType.value ?? {}

  if (code) {
    requests.push(getPartsMasterObjectTypeFields(code))
    requests.push(getPartsMasterObjectTypeFieldValues(props.id, code))
...
const archiveItem = async (item) => {
  ...
  try {
    ...
    await postRequest(
      `/partsMaster/versions/${props.id}/values/${partType.value.code}/${item.pk}/archive`,
      undefined,
      'blueraven'
    )
    ...
    appStore.showSnack(
      'SUCCESS',
      `Row was successfully archived. It will not be available in future versions.`
    )
  } catch (e) {
...
const deleteItem = async (item) => {
  const { data } = await deleteRequestWithPayload(
    `/partsMaster/versions/${props.id}/values/${partType.value.code}/${item.pk}`,
    'blueraven'
  )
...
const undoAllChanges = async () => {
  const { data } = await postRequest(
    `/partsMaster/versions/${props.id}/values/${partType.value.code}/reset`,
    {},
    'blueraven'
  )

  const filteredValues = data.map(({ pk, versionId, row }) => ({
    pk,
    versionId,
    ...row
  }))
  ...
  appStore.showSnack(
    'SUCCESS',
    `All changes to "${partType.value.name}" successfully reverted!`
  )
}
const getPartsMasterDetail = async (partsMasterVersionId) => {
  try {
    const { data } = await getRequestWithParams(
      `/partsMaster/versions/${partsMasterVersionId}`,
      {},
      'blueraven'
    )
...
const getPartsMasterObjectTypes = async () => {
  const { data } = await getRequestWithParams(
...
const getPartsMasterObjectTypeFields = async (objectType) => {
  const { data } = await getRequestWithParams(
    `/partsMaster/versions/fields/${objectType}`,
    {},
    'blueraven'
  )
...
const getPartsMasterObjectTypeFieldValues = async (
  partsMasterVersionId,
  objectType
) => {
  const { data } = await getRequestWithParams(
    `/partsMaster/versions/${partsMasterVersionId}/values/${objectType}`,
    {},
    'blueraven'
  )
  debugger
...
const publish = async (partsMasterVersionId, message) => {
  try {
    const { data } = await postRequest(
      `/partsMaster/versions/${partsMasterVersionId}/publish`,
      { message },
      'blueraven'
    )
    detail.value = { ...data }
    ...
    appStore.showSnack(
      'SUCCESS',
      `Parts Master Version #${partsMasterVersionId} Successfully Published`
    )
  } catch (e) {
    const message = e?.data?.message ?? 'Unable to publish parts master version'
...
const doSaveValues = async (group) => {
  try {
    const { data = {} } = await postRequest(
      `/partsMaster/versions/${detail.value.id}/values/${partType.value.code}`,
      group,
      'blueraven'
    )
    ...
    appStore.showSnack('ERROR', 'Error Updating Parts Master Version Fields')
  }
}
