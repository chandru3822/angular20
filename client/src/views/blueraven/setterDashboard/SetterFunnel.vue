...
const funnelStatsLoading = ref(false)
const areaModel = ref([])
const areaData = ref([])
...
const pipelineDateRange = ref({label: 'Week to Date', value: 'WTD'})
const showPipelineCustomDates = ref(false)
const customDateSelectorIsOpen = ref(false)
const viewSelect = ref('standard')
const pipeline_dt1 = ref(moment().startOf('W').format('YYYY-MM-DD'))
const pipeline_menu1 = ref(false)
const pipeline_dt2 = ref(moment().format('YYYY-MM-DD'))
const pipeline_menu2 = ref(false)
...
const pipeline_dt1_formatted = computed(() => {
  return formatFunnelDate(pipeline_dt1.value)
})
const pipeline_dt2_formatted = computed(() => {
  return formatFunnelDate(pipeline_dt2.value)
})

watch(funnelDrilldownDialog, async () => {
  funnelDrilldownSearch.value = ''
})

const doRepWatcher = () => {
  if (repValuesChanged.value) {
    let useRepDataInstead = isSetter.value ? false : selectAllReps.value
    pipelineLoad(pipeline_dt1.value, pipeline_dt2.value, useRepDataInstead)
...
const regionLoad = async (preSelectLists) => {
  ...
  await getSetterRegions(currentUserId.value, JSON.stringify(areas)).then(res => {
...
const districtLoad = async (preSelectLists) => {
  ...
  // appStore.loading = true
  await getSetterDistricts(currentUserId.value, JSON.stringify(areas), JSON.stringify(regions)).then(res => {
    if (res?.length > 0) {
...
const officeLoad = async (preSelectLists) => {
  ...
  // appStore.loading = true
  await getSetterOffices(currentUserId.value, JSON.stringify(areas), JSON.stringify(regions), JSON.stringify(districts)).then(res => {
...
const repLoad = async (preSelectLists, loadFilterOnFirstLoad) => {
...
const pipelineLoad = async (start, end, useRepDataInstead) => {
  ...
  try {
    const requestBody = {
      ...
      isCohort: viewSelect.value === 'cohort'
    }
...
const choosePipelineDateRange = (dateRange) => {
  ...
  switch (dateRange.value) {
    ...
    case 'YTD':
      ...
      break
    case 'Custom':
      showPipelineCustomDates.value = true
      appStore.loading = false
      break
    default:
      previousNumberOfDays(dateRange.value)
      break
  }
}
...
const yesterday = () => {
  ...
  pipeline_dt2.value = moment().subtract(1, 'd').format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const lastWeek = () => {
  pipeline_dt1.value = moment().subtract(1, 'week').startOf('week').add(1, 'day').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().subtract(1, 'week').endOf('week').add(1, 'day').format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const lastMonth = () => {
  pipeline_dt1.value = moment().subtract(1, 'month').startOf('month').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().subtract(1, 'month').endOf('month').format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const weekToDate = () => {
  pipeline_dt1.value = moment().startOf('isoWeek').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const monthToDate = () => {
  pipeline_dt1.value = moment().startOf('month').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const quarterToDate = () => {
  let quarter = moment().quarter()
  pipeline_dt1.value = moment().startOf('year').quarter(quarter).format('YYYY-MM-DD')
  pipeline_dt2.value = moment().format('YYYY-MM-DD')
  updatePipelineCalendar(true)
}
const yearToDate = () => {
  pipeline_dt1.value = moment().startOf('year').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().format('YYYY-MM-DD')
  updatePipelineCalendar()
}
const previousNumberOfDays = (days) => {
  pipeline_dt1.value = moment().subtract(days, 'days').format('YYYY-MM-DD')
  pipeline_dt2.value = moment().subtract(1, 'days').format('YYYY-MM-DD')
  updatePipelineCalendar()
}
...
const loadFunnel = async () => {
  if (funnelStats.value?.length === 0) {
    //if they are a setter we have to wait for each request first so each list gets filtered
    if (isSetter.value) {
      await areaLoad(true)
      await regionLoad(true)
      await districtLoad(true)
      await officeLoad(true)
      repLoad(true, true)
    } else {
      let requests = [
        areaLoad(false),
...
const funnelDrilldown = async (funnelIdClicked, dateRange, funnelName) => {
  ...
  let orgs = []
  let start, end
  reps = modelOverride.value ? repDataMaster.value.map(rep => rep.user_position_id) : repModel.value.map(rep => rep.user_position_id)
  ...
  switch (dateRange) {
    ...
    case '30days':
      ...
      break
    default:
      start = pipeline_dt1.value
      end = pipeline_dt2.value
      break
  }

  if (moment(start).format('YYYY-MM-DD') === moment(end).format('YYYY-MM-DD')) {
    funnelDrilldownTitle.value = funnelName + ' on ' + moment(start).format('M/D/YYYY')
  } else {
    funnelDrilldownTitle.value = funnelName + ' ' + moment(start).format('M/D/YYYY') + ' - ' + moment(end).format('M/D/YYYY')
  }

  const requestBody = {
    start: start,
    end: end,
    funnelId: funnelIdClicked,
    users: reps,
    orgs: orgs,
    isCohort: viewSelect.value === 'cohort'
  }
...
onMounted(async () => {
  currentUserId.value = userStore.details.id
