...
const topOfficesLoading = ref(false)
...
const loadPersonalPerformance = async () => {
  performanceDataLoading.value = true

  try {
    let startDate = moment().subtract(timeInterval.value, 'd').format('YYYY-MM-DD')
    //dont include today if the selected option is yesterday
    let endDate = timeInterval.value === 1 ? startDate : moment().format('YYYY-MM-DD')
    ...
    if (isSetterMgr.value) {
      ...
      performanceDataLoading.value = false
    } else {
...
const getTopReps = async () => {
  try {
    ...
    const params = {limit: 15, days: timeInterval.value, interval: timeIntervalString.value}
