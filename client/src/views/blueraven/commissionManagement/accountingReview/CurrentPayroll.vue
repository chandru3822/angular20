...
<script setup>
  ...
  import { handleHidingGlobalLoader, getRequest, postRequest, getSnackbar, getRequestWithParams } from '@/helpers/helpers.js'
  ...
  const userCanAdd = computed(() => {
    return userStore.userHasFeatureAccessLevel('COMMISSIONS_CLOSER', 'ADD') ||
      userStore.userHasFeatureAccessLevel('COMMISSIONS_SETTER', 'ADD')

  })
  const userCanEdit = computed(() => {
    return userStore.userHasFeatureAccessLevel('COMMISSIONS_CLOSER', 'EDIT') ||
      userStore.userHasFeatureAccessLevel('COMMISSIONS_SETTER', 'EDIT')
  })
  ...
  watch(commissionPositionId, async () => {
    if ([1,4].includes(commissionPositionId.value)) {
      await getPlans()
      await getCurrentPayroll()
    } else {
      await router.push({ path: '/commissionManagement/accounting/partnerPay' })
    }
  })
