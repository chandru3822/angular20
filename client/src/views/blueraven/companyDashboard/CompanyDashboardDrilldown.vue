...
const { milestone, drilldownData, loadPartners, closeCallback, startDate, endDate, additionalHeaders } = toRefs(props)
...
const drilldownRowCount = ref(0)
const filteredDrilldownData = ref([])


const filteredDrilldownItems = (filteredItems) => {
  filteredDrilldownData.value = filteredItems
  drilldownRowCount.value = filteredItems.length
}
...
const exportCsv = () => {
  ...
  filteredHeaders.value?.forEach(h => {
    if(h.text !== '') {
      return csv += `${h.text},`
    }
  })
