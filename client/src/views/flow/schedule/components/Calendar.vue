<template>
  <div id="calendar-container">
    <MessagingDialog
      ...
      :user-id-to-message="userToMessage?.userId"
      :title="userToMessage?.title"
      :center-left="mapOpen"
      @close="[(userToMessage = null)]"
    />
    <div
      id="calendar-filter-container"
      v-show="!$vuetify.breakpoint.smAndDown || showFilters"
      class="pa-6 pt-1"
      :class="{ 'background-clear': isSidebarView }"
    >
      <v-col cols="11" class="pa-0">
        ...
        <v-row class="py-0 d-flex align-baseline">
          ...
          >
            ...
            >
              <template v-slot:selection="{ item, index }">
                <div v-if="index === 0 && selectedStates.length < 3">
                  <v-chip
                    small
                    close
                    @click:close="selectedStates.splice(idx, 1)"
              ...
              <template v-slot:prepend-item>
                <v-list-item ripple @click="toggleSelectAllStates()">
                  <v-icon class="mr-4">{{ iconStates }}</v-icon>
                  <v-list-item-title class="wrap-dropdown-item py-2">
                    Select All
                  </v-list-item-title>
                </v-list-item>
              ...
              <template v-slot:item="{ item }">
                <v-icon class="mr-4">
                  {{
                    selectedStates.findIndex(
                      (s) => s.stateId === item.stateId
                    ) >= 0
                      ? 'check_box'
          ...
          <v-col
            id="org-resource-types-filter-col"
            cols="12"
            sm="4"
            md="3"
            :lg="mapOpen ? '4' : '2'"
          >
            ...
            >
              <template v-slot:selection="{ item, index }">
                <div v-if="index === 0 && selectedOrgTypes.length < 3">
                  <v-chip
                    small
                    close
                    @click:close="selectedOrgTypes.splice(idx, 1)"
              ...
              <template v-slot:prepend-item>
                <v-list-item ripple @click="toggleSelectAllOrgTypes()">
              ...
              <template v-slot:item="{ item }">
                ...
                <v-icon class="mr-4">
                  {{
                    selectedOrgTypes.findIndex((ot) => ot.id === item.id) >= 0
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  }}
                </v-icon>
          ...
          >
            <a-autocomplete
              ...
              :error-messages="
                countSelected >= maxSelectionAllowed ? countErrorMessage : null
              "
              return-object
              item-title="orgName"
              item-value="id"
            ...
            >
              ...
              <template v-slot:item="{ item }">
                ...
                <v-icon class="mr-4">
                  {{
                    selectedOrgs.findIndex((o) => o.id === item.id) >= 0
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  }}
                </v-icon>
          ...
          <v-col
            id="placeholder-desktop-col"
            v-if="$vuetify.breakpoint.md && !mapOpen"
            cols="0"
            md="3"
            class="py-0"
          />
          <v-col
            id="position-resource-types-col"
            cols="12"
            sm="4"
            md="3"
            :lg="mapOpen ? '4' : '2'"
          >
            ...
            >
              <template v-slot:selection="{ item, index }">
                <div v-if="index === 0 && selectedPositions.length < 3">
                  <v-chip
                    small
                    close
                    @click:close="selectedPositions.splice(idx, 1)"
              ...
              <template v-slot:prepend-item>
                <v-list-item ripple @click="toggleSelectAllPositions()">
                  <v-list-item-action class="mr-4">
              ...
              <template v-slot:item="{ props, item }">
                ...
                <v-icon class="mr-4">
                  {{
                    selectedPositions.findIndex((p) => p.id === item.id) >= 0
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  }}
                </v-icon>
          ...
          >
            ...
            >
              ...
              <template v-slot:item="{ item }">
                ...
                <v-icon class="mr-4">
                  {{
                    selectedUsers.findIndex((u) => u.id === item.id) >= 0
                      ? 'check_box'
                      : 'check_box_outline_blank'
                  }}
                </v-icon>
          ...
          <v-col
            id="cancelled-events-toggle-col"
      ...
      >
          ...
          <div class="d-flex justify-space-between align-baseline">
            <span>
              <a-btn
                ...
                icon
                size="small"
                @click="collapseExpandResources"
              >
                <v-icon v-if="resourcesExpandedMobile">
              ...
              >
                <template v-slot:activator="{ on }">
                  <a-btn
                    icon
                    size="small"
                    @click="toggleMapPinsForAllResources(!allResourcesOnMap)"
                    :activation-handler="on"
                    class="mx-1"
                  >
                    <v-icon color="grey darken-3" v-if="allResourcesOnMap">
                      mdi-map-marker
                    </v-icon>
        ...
        <template v-slot:resourceLabelContent="{ resource, index }">
          <div class="d-flex justify-space-between align-baseline flex-wrap">
            <a
              v-if="
                resource.id.charAt(0) === '1' &&
                userStore.userHasFeature('USERS')
              "
              :href="`${getHostUrl()}/org/${resource.id.substring(1)}`"
              target="_blank"
              class="body-large overflow-hidden resource-title text-decoration-none"
              >{{ resource.title }}</a
            >
            <a
              v-else-if="
                resource.id.charAt(0) !== '1' &&
                userStore.userHasFeature('ORGS')
              "
              :href="`${getHostUrl()}/user/${resource.id.substring(1)}/details`"
              target="_blank"
              class="body-large overflow-hidden resource-title text-decoration-none"
            >
              {{ resource.title }}
            </a>
            <span v-else class="body-large overflow-hidden resource-title">
              {{ resource.title }}
            </span>
            <div class="text-right flex-grow-1">
              ...
              >
                <template v-slot:activator="{ on }">
                  <a-btn
                    icon
                    size="small"
                    @click="toggleMapPinForResource(resource)"
                    :activation-handler="on"
                    class="mx-1"
                  >
                    <v-icon
                      :color="resource.extendedProps.color"
                      v-if="isResourceOnMap(resource) || allResourcesOnMap"
                    >
                      mdi-map-marker
                    </v-icon>
                    <v-icon color="grey darken-1" v-else>
                ...
                <span v-if="isResourceOnMap(resource)">
                  Remove pin from map
                </span>
                ...
                <template v-slot:activator="{ on }">
                  <a-btn
                    v-if="
                      !isSidebarView &&
                      userCanSms &&
                      resource.id.charAt(0) === '2' &&
                      resource.extendedProps.hasSMSAccess
                    "
                    icon
                    size="small"
                    @click="
                      [
                        (userToMessage = {
                          userId: Number(resource.id.substring(1)),
              ...
              >
                <template v-slot:activator="{ on }">
                  <a-btn
                    v-if="showScheduleBtnForResource(resource)"
                    icon
                    size="small"
                    :color="
                    ...
                    "
                    class="mx-1"
                    @click="toggleScheduleResource(resource)"
                    :activation-handler="on"
                  >
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <span
                ...
                v-bind="attrs"
                v-on="on"
                :class="{ 'text-no-wrap': event.display !== 'background' }"
                class="event-title body-medium"
                >{{ event.title }}</span
              >
            </template>
...
const collapseExpandResources = () => {
  ...
  calendarApi.setOption(
    'resourceAreaWidth',
    vuetify.breakpoint.smAndDown && !resourcesExpandedMobile.value ? '25%' : 300
  )
}

const eventCalendar = ref(null)
const userCanEdit = computed(() =>
  userStore.userHasFeatureAccessLevel('SCHEDULE', 'EDIT')
)
...
const calendarOptions = ref({
  plugins: [resourceTimelinePlugin, interaction, momentTimezonePlugin],
  firstDay: 1,
  ...
  eventSources: [
    (info, successCallback, failureCallback) =>
      goGetEventsNow(info, successCallback, failureCallback)
  ],
  ...
  views: {
    resourceTimelineDay: {
      titleFormat: {
        month: vuetify.breakpoint.smAndDown ? 'short' : 'long',
        year: 'numeric',
        day: 'numeric',
...
const sortedStates = computed(() => {
  const sStates = props.states.filter((state) =>
    selectedStates.value.includes(state)
  )
  const uStates = props.states.filter(
    (state) => !selectedStates.value.includes(state)
  )
...
const sortedOrgTypes = computed(() => {
  if (!Array.isArray(orgTypes.value)) {
    return []
  }

  const sots = orgTypes.value?.filter((orgType) =>
    selectedOrgTypes.value.includes(orgType)
  )
  const usots = orgTypes.value?.filter(
    (orgType) => !selectedOrgTypes.value.includes(orgType)
  )
...
const sortedPositions = computed(() => {
  ...
  const sp = positions.value.filter((position) =>
    selectedPositions.value.includes(position)
  )
  const up = positions.value.filter(
    (position) => !selectedPositions.value.includes(position)
  )
  return sp.concat(up)
})
...
const sortedUsers = computed(() => {
  ...
  const sUsers =
    users.value?.length > 0
      ? users.value?.filter((user) => selectedUsers.value?.includes(user))
      : []
  const usUsers =
    users.value?.length > 0
      ? users.value?.filter((user) => !selectedUsers.value?.includes(user))
  ...
  (value) => {
    ...
    if (userTimezone.value !== scheduleTimezone.value) {
      const snackbar = createSnackbar(
        'Note: Timezone changes only affect the scheduling tool.  The timezone everywhere else on Albatross remains unchanged.'
      )
...
watch(selectedUsers, (newValue, oldValue) => {
  calendarOptions.value.resources = selectedOrgs.value.concat(
    selectedUsers.value
  )
  if (oldValue.length > newValue.length) {
    //if we're removing users
    const removedUsers = oldValue.filter(
      (oldUser) => newValue.indexOf(oldUser) < 0
    )
...
watch(selectedOrgs, (newValue, oldValue) => {
  ...
  if (oldValue.length > newValue.length) {
    //if we're removing users
    const removedOrgs = oldValue.filter(
      (oldOrg) => newValue.indexOf(oldOrg) < 0
    )
...
const isAssignedResource = (resource) => {
  ...
  if (resource.extendedProps.orgId === selectedResourceId) {
    ...
    const positions = resource.extendedProps.userPositions?.filter(
      (p) => p.id === selectedResourceId
    )
...
const showScheduleBtnForResource = (resource) => {
  ...
  if (
    props.preselectedEvent &&
    userCanEdit &&
    props.preselectedEvent.editableInSchedule
  ) {
    if (props.preselectedEvent.systemListId === 2) {
      const allowedPositions = resource.extendedProps?.userPositions?.filter(
        (p) => props.preselectedEvent.systemListOptionIds.includes(p.positionId)
      )
      return allowedPositions?.length > 0
    } else if (props.preselectedEvent.systemListId === 3) {
      return props.preselectedEvent.systemListOptionIds.includes(
        resource.extendedProps.orgTypeId
      )
    }
  }
  return false
  ...
  try {
    ...
    if (props.preselectedEvent) {
      if (props.preselectedEvent.systemListId === 3) {
        selectedOrgTypes.value = orgTypes?.value.filter((ot) =>
          props.preselectedEvent.systemListOptionIds.includes(ot.id)
        )
      }
    }
...
const getPositions = async () => {
  ...
  try {
    ...
    if (props.preselectedEvent) {
      if (props.preselectedEvent.systemListId === 2) {
        selectedPositions.value = positions.value.filter((p) =>
          props.preselectedEvent.systemListOptionIds.includes(p.id)
        )
      }
    }
  ...
  if (
    ...
    orgs.value = masterOrgs.value.filter((mo) => {
      ...
      if (orgTypeFilterRequired) {
        let match = selectedOrgTypes.value.find(
          (sot) => sot.id === mo.orgTypeId
        )
...
const handlePinsOnDayChange = () => {
  ...
  checkedResources.value.forEach((r, idx) => {
    handlePopulatingMapPins(
      true,
      r,
      false,
      idx === checkedResources.value.length - 1
    )
  })
}
const handlePopulatingMapPins = (addPin, resource, doCallback) => {
  if (addPin) {
    ...
    let resourceEvents = calendarApi.getEvents().filter((e) => {
      return (
        e.display !== 'inverse-background' &&
        e.display !== 'background' &&
        e._def.resourceIds.indexOf(resource.id) >= 0
      )
    })
    ...
    mapPinnedResources.value = mapPinnedResources.value.filter(
      (rId) => resource?.id !== rId
    )
  }
  if (doCallback) {
    ...
    let params = {
      orgIds:
        selectedOrgs.value?.length > 0
          ? selectedOrgs.value.map((o) => o.masterId)
          : [],
      userIds:
        selectedUsers.value?.length > 0
          ? selectedUsers.value.map((u) => u.masterId)
          : [],
    ...
    data?.forEach((d) => {
      ...
      if (d.display === 'background' || d.display === 'auto') {
        ...
        d.title =
          d.title +
          ': ' +
          getFormattedDate(d.start, 'hh:mm') +
          '-' +
          getFormattedDate(d.end)
...
const updatePins = async (info) => {
  let filteredResources = mapResourceEvents.value?.filter((e) =>
    isValidEventDate(
      Date.parse(e.start),
      Date.parse(e.end),
      Date.parse(info.start),
      Date.parse(info.end)
    )
  )
    ...
    try {
      ...
      data.forEach((d) => {
        ...
        d.resourceId = d.userId
          ? `${d.systemListTypeId}${d.userId}`
          : `${d.systemListTypeId}${d.resourceId}`
        ...
        let matchingResource = calendarOptions.value.resources.find(
          (r) => r.id === d.resourceId
        )
        ...
        if (
          isResourceOnMap(matchingResource) &&
          d.projectProcessStepEventId &&
          isValidEventDate(
            Date.parse(d.start),
            Date.parse(d.end),
            Date.parse(calendarStartTime.value),
            Date.parse(calendarEndTime.value)
          )
        ) {
          let eventObj = {
            key:
...
const handleEventClick = (info) => {
  if (
    info.event.title &&
    info.event.display === 'auto' &&
    info.event.extendedProps?.projectProcessStepId
  ) {
    let props = info.event.extendedProps
    //open event clicks in new window every time so they dont have to keep reloading the calendar
    let routerData = router.resolve({
      path: `/project/${props.projectId}/processStep/${props.projectProcessStepId}/event/${props.projectProcessStepEventId}`
    })
...
#event-calendar
  ...
  > table
  > tbody
  > tr.fc-timeline-header-row.fc-timeline-header-row-chrono
  > th {
  padding-bottom: 8px;
}
...
#calendar-container
  ...
  > tbody
  > tr
  > td.fc-time-area.fc-widget-content
  > div
  > div
  > div
  > div.fc-content
...
#event-calendar
  > div.fc-view-harness.fc-view-harness-active
  > div.fc-resourceTimelineWeek-view.fc-view.fc-resource-timeline.fc-resource-timeline-flat.fc-timeline.fc-timeline-overlap-enabled
  > table
  > tbody
  > tr
  > td:nth-child(3)
