<template>
  <v-container id="work-queue-drilldown-container">
    <v-row v-if="!dataLoading && !errorLoading">
      <v-col cols="12">
        <v-toolbar flat class="app-toolbar">
          ...
          <v-toolbar-title
            class="app-title"
            v-if="workQueue && workQueue.workQueueType"
            >{{ workQueue.workQueueType }}
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-switch
        ...
        <div
          v-if="noResults && !dataLoading"
          class="one-hunned text-center mt-5"
        >
          No results found
        </div>
        <v-data-table
        ...
        >
          ...
          <template #item="{ item, index }">
            <tr
              class="clickable v-data-table-row"
              :class="{ 'shaded-row': index % 2 }"
            >
              <td
                class="text-left pl-1"
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('Project Name')
                "
              ...
              <td
                class="text-left"
                v-if="
                  useProcessStepHeaders && !hiddenHeaders.includes('Project ID')
                "
              >
                <router-link
              ...
              <td
                ...
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('Process Step Name')
                "
              >
                <router-link
                  class="router-link-td elevation-0 square-card"
              ...
              <td
                ...
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('Process Step Status Type')
                "
              >
                <router-link
                  class="router-link-td elevation-0 square-card"
              ...
              <td
                ...
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('Days In Queue')
                "
              >
                <router-link
                  class="router-link-td elevation-0 square-card"
              ...
              <td
                ...
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('State Abbreviation')
                "
              >
                <router-link
                  class="router-link-td elevation-0 square-card"
              ...
              <td
                class="text-left"
                v-if="useProcessStepHeaders && !hiddenHeaders.includes('Owner')"
              >
                <router-link
                  v-if="item['Owner']"
                  class="router-link-td elevation-0 square-card"
              ...
              <td
                ...
                v-if="
                  useProcessStepHeaders &&
                  !hiddenHeaders.includes('Active Process Steps')
                "
              >
                <router-link
                  class="router-link-td elevation-0 square-card"
              ...
              <template v-for="c in customColumns">
                <td
                  v-if="
                    !hiddenHeaders.includes(c.name) ||
                    customHeaders.includes(c.name)
                  "
                >
                  <div
                    v-if="!useProcessStepHeaders && c.name === 'Project Name'"
    ...
    <ConfirmationDialog
      :open-dialog="showNotesModal"
      hide-confirm
      @confirm="updateRowNotes"
      @close-dialog="showNotesModal = false"
      :width="1800"
      :primary-header="true"
    >
      <template v-slot:title
        >{{ itemToUpdate ? itemToUpdate['Project Name'] : '' }} -
        {{ itemToUpdate ? itemToUpdate['Process Step Name'] : '' }}</template
      >
      ...
      <NotesAndActivityContent
        ...
        :primary-id="
          itemToUpdate
            ? workQueue.useEventData
              ? itemToUpdate.projectProcessStepEventId
              : itemToUpdate.projectProcessStepId
            : null
        "
        :secondary-id="
          itemToUpdate
            ? workQueue.useEventData
              ? itemToUpdate.processStepEventWorkQueueTypeId
              : itemToUpdate.processStepWorkQueueTypeId
            : null
        "
...
const customHeaders = computed(() => {
  console.log(
    'Custom Headers',
    headers.value.filter((h) => h?.isDefault === false)
  )
  return headers.value
    .filter((h) => h?.isDefault === false)
    .map((h) => h?.value)
})

onMounted(async () => {
  cachedFilters.value =
    JSON.parse(localStorage.getItem('wqDrilldownFilters')) || {}
  hideFutureFollowUps.value =
...
const sortOrderingInit = async () => {
  let columnsToSort = workQueue.value?.columnSortingOrder?.filter(
    (cso) => cso.value !== null && cso.value !== ''
  )
...
const getWorkQueueName = async () => {
  ...
  try {
    const { data, status } = await getRequest(
      `/workQueueType/${workQueueTypeId.value}`
    )
...
const filterFutureFollowUps = () => {
  localStorage.setItem(
    'hideFutureWqFollowUps',
    JSON.stringify(hideFutureFollowUps.value)
  )
  localStorage.setItem(
    'hideFutureWqEvents',
    JSON.stringify(hideFutureEvents.value)
  )
  ...
  if (!workQueue.value.useEventData && hideFutureFollowUps.value) {
    results.value = filteredResults.value.filter((r) => {
      if (r.notes && r.notes.length > 0) {
        ...
        let noFollowUp =
          firstNoteFollowUp === null || firstNoteFollowUp === undefined
        ...
        return (
          ...
          (!noFollowUp &&
            moment().isAfter(
              moment.utc(firstNoteFollowUp, 'YYYY-MM-DD').format('YYYY-MM-DD')
            ))
        )
      } else {
        return true
    ...
    workQueue.value.useEventData &&
    (hideFutureEvents.value || hideFutureFollowUps.value)
  ) {
    results.value = filteredResults.value.filter((r) => {
      let noteFilter = true
      if (r.notes && r.notes.length > 0) {
      ...
      return (
        (!hideFutureFollowUps.value ||
          (hideFutureFollowUps.value && noteFilter)) &&
        (!hideFutureEvents.value ||
          (hideFutureEvents.value &&
            moment(r['Event Start Time'], 'MM/DD/YYYY hh:mm a').isBefore(
              moment()
            )))
      )
    })
...
const exportCsv = async () => {
  try {
    ...
    const { data, status } = await getRequestWithParams(
      `/smartlistv1/${smartlistId.value}/csv`,
      {
        params: {
    ...
    saveAs(
      blob,
      `${workQueue.value.workQueueType} ${DateTime.local().toFormat('yyyy-MM-dd h_mm a')}.csv`
    )
    handleHidingGlobalLoader(status)
  } catch (e) {
...
const getWorkDetails = async () => {
  ...
  try {
    ...
    useProcessStepHeaders.value =
      results.value?.length > 0 && 'Owning Positions' in results.value[0]
    ...
    if (results.value?.length > 0 && 'Notes' in results.value[0]) {
      ...
      results.value.forEach((r) => {
        ...
        ;(r.followUpDate =
          null != r.notes[0]?.followUpDate &&
          undefined !== r.notes[0]?.followUpDate
            ? moment
                .utc(r.notes[0]?.followUpDate, 'YYYY-MM-DD')
                .format('M/D/YYYY')
            : null),
          (r.firstNoteCreatedAt = r.notes[0]?.dateCreated),
          (r.firstNoteCreatedAtFormatted =
            null != r.notes[0]?.dateCreated &&
            undefined !== r.notes[0]?.dateCreated
              ? moment
                  .utc(r.notes[0]?.dateCreated, 'YYYY-MM-DDTHH:mm:ssZ')
                  .tz(timezone.value)
    ...
    if (useProcessStepHeaders.value) {
      headers.value = [
        ...
        {
          text: 'Status',
          value: 'Process Step Status Type',
          show: true,
          isDefault: true
        },
        {
        ...
        {
          text: 'Active Process Steps',
          value: 'Active Process Steps',
          show: true,
          isDefault: true
        }
      ]
      ...
      headers.value.push({
        ...
        sort: (a, b) => {
          ...
          //this does a lot of extra checking we probably dont need now that we know the data type id of the column, but i am keeping it cuz i am not sure what it all does
            ...
            [1, 2].includes(c.dataTypeId) &&
            ((null != a && a.toString().match(/^\d{4}-\d{2}-\d{2}/)) ||
              (null != b && b.toString().match(/^\d{4}-\d{2}-\d{2}/)) ||
              (null != a && !isNaN(Date.parse(a))) ||
              (null != b && !isNaN(Date.parse(b))))
          ) {
            ...
            return (
              (null != a ? new Date(a.replace(/-/g, '/')) : a) -
              (null != b ? new Date(b.replace(/-/g, '/')) : b)
            )
          } else {
...
const assignToUser = async (item) => {
  try {
    ...
    const { status } = await postRequest(
      `/projectProcessStep/${item.projectProcessStepId}/owner/checkExisting`,
...
const userCanOwnProcessStep = (item) => {
  ...
  item.owningPositions?.forEach((op) => {
    let positionMatch = userPositions.value.find(
      (up) => up.positionId === op.positionId
    )
    if (positionMatch !== null && positionMatch !== undefined) {
      canAssign = true
      userPositions.value.find(
        (up) => up.positionId === op.positionId
      ).canAssign = true
    }
  })
...
const clickRow = (row) => {
  router.push({
    path: `/project/${row.projectId}/processStep/${row.projectProcessStepId}`
  })
}
const filterResults = () => {
  results.value = masterResults.value.filter((r) => {
    ...
    Object.keys(filters.value).forEach((key) => {
      ...
      let value =
        filters.value[key].trim().length === 0 ? '' : filters.value[key]
    ...
    localStorage.setItem(
      'wqDrilldownFilters',
      JSON.stringify(cachedFilters.value)
    )
...
const updateRowNotes = (item, isNew) => {
  ...
  let matchInFilteredResults = filteredResults.value.find(
    (fr) =>
      fr.projectProcessStepEventId ===
        results.value[notesPpsIndex.value].projectProcessStepEventId &&
      fr.processStepEventWorkQueueTypeId ===
        results.value[notesPpsIndex.value].processStepEventWorkQueueTypeId
  )

  let followUpDate =
    null != item.followUpDate
      ? moment.utc(item.followUpDate, 'YYYY-MM-DD').format('M/D/YYYY')
      : null
  ...
  let dateFormatted =
    null != item.dateCreated
      ? moment
          .utc(item.dateCreated, 'YYYY-MM-DDTHH:mm:ssZ')
          .tz(timezone.value)
          .format('M/D/YYYY h:mm a')
      : null
...
#wq-drilldown-table
  ...
  > tr
  > td
  > button.primary--text.v-btn--outlined {
