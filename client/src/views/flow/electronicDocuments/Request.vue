    ...
    methods: {
      async fetchProjects() {
        try {
            ...
            const {data} = await getRequestWithParams(`/electronicDocument/projects`, { params: {
      ...
      async fetchTemplates () {
          try {
              ...
              if (this.selectedTempType == 'Permitting') {
                  const {data} = await getRequest('/electronicDocument/getPermittingDocuments/'+this.project_id)
                  if (data != null) {
                      this.documents = data
                  }
              }
