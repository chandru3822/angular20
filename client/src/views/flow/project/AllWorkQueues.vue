<template>
  <v-row
    no-gutters
    id="project-details-wq-container"
  ...
  >
    <v-col cols="12" lg="12" class="pa-3">
      <v-toolbar color="transparent" class="elevation-0 project-wq-toolbar">
        ...
        <v-toolbar-items>
          <a-btn
            ...
            :prepend-icon="
              !expandCurrent ? 'mdi-chevron-down' : 'mdi-chevron-up'
            "
          ></a-btn>
        </v-toolbar-items>
      </v-toolbar>
      ...
      <v-card class="square-card" v-if="expandCurrent">
        ...
        >
          <template #no-data> No current work queues </template>

          <template #no-results> No current work queues </template>

          <template #item.workQueueCategory="{ item }" class="text-left">
            {{ item.workQueueCategory }}
          </template>
          <template #item.workQueueType="{ item }" class="text-left">
            {{ item.workQueueType }}
          </template>
          <template #item.daysInQueue="{ item }" class="text-left">
            {{ item.daysInQueue }}
          </template>
          <template #item.processStepName="{ item }" class="text-left">
      ...
      <v-toolbar
        color="transparent"
        class="elevation-0 mt-6 project-wq-toolbar"
      >
        <v-toolbar-title class="albatross-header-3"
          >Historic Work Queues</v-toolbar-title
        >
        <v-spacer></v-spacer>
        <v-toolbar-items>
          <a-btn
            ...
            :prepend-icon="
              !expandHistoric ? 'mdi-chevron-down' : 'mdi-chevron-up'
            "
          ></a-btn>
        </v-toolbar-items>
      </v-toolbar>
...
const currentWorkQueues = computed(() => {
  return Array.isArray(workQueueHistory.value)
    ? workQueueHistory.value?.filter(
        (wqh) => wqh.status === 'Currently in Queue'
      )
    : []
})
const historicWorkQueues = computed(() => {
  return Array.isArray(workQueueHistory.value)
    ? workQueueHistory.value?.filter(
        (wqh) => wqh.status === 'Work Queue History'
      )
    : []
})
...
const getWorkQueueHistory = async () => {
  try {
    ...
    const { data } = await getRequest(
      `/project/${projectId.value}/workQueueHistory`
    )
