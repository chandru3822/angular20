    ...
    <v-row class="pt-0">
      <v-col cols="12" class="pt-0">
        <v-toolbar flat dense>
          <v-toolbar-title> Slot Schedules </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <a-btn
              variant="text"
        ...
        <v-card v-if="addNew" class="mb-2 pa-5">
          ...
          ></a-text-field>
          <a-btn
            :disabled="!newSchedule.scheduleName"
        ...
        >
          ...
          <template #expanded-item="{ headers, item }">
            <td
              ...
              class="pa-4"
              :class="{ 'shaded-row': slotSchedules.indexOf(item) % 2 }"
            >
              <h3>Edit Schedule</h3>
              <a-text-field v-model="item.scheduleName" label="Schedule Name" />
              <div class="mb-3">
                <a-btn
                  color="primary"
                  class="mb-2"
                ...
                />
                <v-list
                  v-for="(st, index) in item.slotTimes.filter(
                    (a) => !a.archived
                  )"
                  :key="index"
                  class="pa-0"
...
const filterSchedules = computed(() => {
  return slotSchedules.value.filter((s) => {
    return !s.archived
  })
})
    ...
    schedule.slotTimes
      ...
      ?.forEach((st, stIdx) => {
        ...
        if (!st.archived && (!st.startTime || !st.endTime)) {
          ...
          moment(st.startTime, 'HH:mm').isSameOrAfter(
            moment(st.endTime, 'HH:mm')
          )
        ) {
          saveError.value = true
          ...
          schedule.slotTimes
            ...
            ?.forEach((st2, st2Idx) => {
              if (
                stIdx !== st2Idx &&
                (moment(st.startTime, 'HH:mm').isBetween(
                  moment(st2.startTime, 'HH:mm'),
                  moment(st2.endTime, 'HH:mm')
                ) ||
                  moment(st.endTime, 'HH:mm').isBetween(
                    moment(st2.startTime, 'HH:mm'),
                    moment(st2.endTime, 'HH:mm')
                  ))
              ) {
...
const deleteSchedule = async () => {
  ...
  try {
    const { status } = await deleteRequest(
      `/availability/slotSchedule/${schedule.id}`
    )
