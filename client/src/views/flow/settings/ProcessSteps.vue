  ...
  <v-container id="process-step-container" class="custom-field-group-container">
    <v-dialog v-model="deleteError">
      <v-card>
        ...
        <v-card-text>
          You cannot delete a process step with fields that are currently in
          use. Please remove any field from the following locations before
          deleting.
            ...
            >
              ...
              <template #item.icons="{ item }">
                ...
                <v-tooltip
                  top
                  :disabled="
                    !(item.workQueueTypes.length > 0 || item.usedByProcess)
                  "
                >
    ...
    <ConfirmationDialog
      :open-dialog="!!psToDelete"
      @confirm="deleteProcessStep"
      @close-dialog="psToDelete = null"
    >
...
const deleteProcessStep = async () => {
  ...
  try {
    const { data, status } = await putRequest(
      `/processStep/delete/${processStep.id}`,
      null,
      null,
      []
    )
...
const filterProcessSteps = computed(() => {
  return processSteps.value.filter((ps) => {
    return !ps.archived
  })
})
