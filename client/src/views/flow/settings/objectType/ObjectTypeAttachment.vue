<template>
  <v-container class="pt-0" v-if="!attachmentLoading">
    ...
    <v-row>
      <v-col cols="12" class="pt-0 px-0">
        ...
        <v-card
          v-if="createNew"
          text
          class="text-left one-hunned pa-3 square-card add-new"
          flat
        ...
        <v-row>
          <v-col cols="12">
            ...
            >
              <template #no-data> No custom field groups assigned </template>

              <template #no-results> No custom field groups assigned </template>

              <template #item.draggable="{ item, index }">
                <td
                  class="draggable-handle-col"
                  :class="{ 'shaded-row': index % 2 && !isMobile }"
                >
                  <a-btn
                    variant="text"
                    icon
                  ...
                  <div v-if="userCanEdit" :class="{ 'shaded-row': index % 2 }">
                    <a-text-field v-if="item.edit" v-model="item.groupName">
                      <template v-slot:append-outer>
                        <v-icon
                          @click=";[saveGroupName(item), (item.edit = false)]"
                        >
                          save
                        </v-icon>
              ...
              <template #expanded-item="{ headers, item, index }">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <v-list
                        v-for="(cf, index) in item.customFields.filter(
                          (a) => !a.archived
                        )"
                        :key="index"
                        class="pa-0"
                        ...
                        <v-list-item :class="{ grab: !item.attachmentTypeId }">
                          ...
                          <v-list-item-content>
                            ...
                            <div v-else>
                              ...
                              {{ cf.groupName }} -
                              {{ cf.fieldName }} (Ancillary)
                            </div>
                          </v-list-item-content>
                          <v-list-item-action
                            v-if="userCanEdit"
                            class="clickable"
                            @click="customFieldToDelete = cf"
                          >
    ...
    <ConfirmationDialog
      :open-dialog="!!cfGroupToDelete"
      @confirm="deleteWithChecks"
      @close-dialog="cfGroupToDelete = null"
    >
      <span class="error--text">WARNING:</span>
      By deleting a Custom Field Group you will lose all data associated with
      native fields in the group.<br />
      Are you sure you want to delete this Custom Field Group:
      <strong>{{ groupToDeleteName }}</strong
      >?<br />
    </ConfirmationDialog>
    <ConfirmationDialog
      :open-dialog="!!customFieldToDelete"
    ...
    >
      ...
      <strong>{{ fieldToDeleteName }}</strong> from
      <strong>{{ fieldToDeleteGroupName }}</strong
      >?
    </ConfirmationDialog>
  </v-container>
</template>
...
const companyObjectTypeId = ref(
  parseInt(route.query.companyObjectTypeId) || parseInt(route.params.id)
)
...
const getTypeDetails = async () => {
  try {
    ...
    const { data } = await getRequest(
      `/attachmentType/${attachmentTypeId.value}/${objectType.value}`
    )
...
const saveFieldGroup = async () => {
  ...
  try {
    ...
    const { data } = await postRequest(
      `/customFieldGroup/addAttachmentCustomFieldGroup`,
      newGroup.value
    )
...
const fetchAvailableCustomFields = async () => {
  try {
    ...
    const { data } = await getRequestWithParams(
      `/processStep/getParentObjectsWithTypes`,
      { params: { id: processStepId.value } }
    )
...
const deleteWithChecks = async () => {
  let item = cfGroupToDelete.value
    ? cfGroupToDelete.value
    : customFieldToDelete.value
    ? customFieldToDelete.value
    : null
  let customFieldGroupId = cfGroupToDelete.value
    ? cfGroupToDelete.value.id
    : null
  let customFieldGroupAssignmentId = customFieldToDelete.value
    ? customFieldToDelete.value.id
    : null
  try {
    appStore.loading = true
    let params = {
...
const saveRowChanges = async (rows) => {
  if (rows?.length > 0) {
    try {
      ...
      localCustomFieldGroups.value = orderBy(
        localCustomFieldGroups.value,
        'groupOrder'
      )
