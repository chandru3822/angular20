    ...
    <v-row>
      <v-col class="shrink" cols="12">
        <v-toolbar flat class="app-toolbar">
          <v-toolbar-title v-if="!isMobile" class="app-title">
            Data Views
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <a-btn
              ...
              color="primary"
              @click="
                ;[(addNew = !addNew), (newDataView = {}), getCompanyProcesses()]
              "
        ...
        <v-card v-if="addNew" class="pa-5 mb-2">
          ...
          <div class="mb-3">
            <v-form ref="dataViewForm">
              <a-text-field
                v-model="newDataView.displayName"
                :rules="requiredRules"
                label="Display Name"
              />
              <a-text-field
                v-model="newDataView.viewName"
                :rules="tableNameRule"
          ...
          <a-btn
            :disabled="
              !newDataView.displayName ||
              !newDataView.viewName ||
              selectedCompanyProcesses.length === 0
            "
            color="primary"
        ...
        <v-data-table
          ...
          class="elevation-1"
        >
          <template #no-data> NO DATA HERE! </template>
          ...
          <template #item="{ item }">
            <tr
              ...
              :class="{ 'shaded-row': dataViews.indexOf(item) % 2 }"
            >
              <td class="text-left">{{ item.displayName }}</td>
              <td class="text-left">{{ item.viewName }}</td>
              <td class="text-right">
                <a-btn variant="text" color="primary" prepend-icon="edit">
                </a-btn>
              </td>
            </tr>
          </template>
...
const tableNameRule = ref([
  () =>
    (newDataView.value.viewName != null && newDataView.value.viewName !== '') ||
  ...
  (v) =>
    !v ||
    (v && v.indexOf('__') <= 0) ||
    "All word dividers must be a single '_'",
  (v) =>
    !v ||
    /^[a-z]+(?:_+[a-z]+)*$/.test(v) ||
    "Table Name must be all lowercase, no symbols except '_' and must start and end with a letter",
  (v) =>
    !v ||
    (v && !constants.RESERVED_SQL_WORDS.includes(v)) ||
...
const saveDataView = async (dv, isNew) => {
  ...
  try {
    ...
    appStore.showSnack(
      'ERROR',
      isNew ? 'Error Adding Data View' : 'Error Updating Data View'
    )
    appStore.loading = false
  }
}
