<template>
  <v-container id="postal-codes" class="custom-field-group-container">
    <v-row>
      <v-col>
        <v-app-bar flat class="elevation-1 call-group-bar">
          <v-toolbar-title class="pt-2 title-large"
            >Call Groups
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            ...
            <a-btn
              variant="text"
              color="primary"
              @click=";[(addNew = !addNew), (newCallGroup = {})]"
              v-if="userCanAdd"
          ...
          <template slot="extension">
            <div v-if="editGroup">
              <a-text-field
                class="d-inline-block mt-4 edit-text"
        ...
        <v-container>
          <v-card v-if="addNew" class="pa-6 mb-2">
            <a-text-field
          ...
          <v-card class="square-card">
            ...
            >
              <template
                #item.callGroupName="{ item }"
                class="text-left clickable"
                @click="goToCallGroup(item.id)"
              >
                <img
                  v-if="item.maxCallCountHit"
                  name="userImg"
                  src="../../../../assets/blueraven/alert_icon.jpg"
                  class="icon-height"
                  title="All active phone numbers exceed max call count"
                />
                {{ item.callGroupName }}
              </template>
              <template #item.activePostalCodes="{ item }">{{
                item.postalCodesCount
              }}</template>
              <template #item.activePhoneNumbers="{ item }">{{
                item.activePhoneNumbersCount
              }}</template>
    ...
    <ConfirmationDialog
      :open-dialog="!!callGroupToDelete"
      @confirm=";[(callGroupToDelete.archived = true), deleteCallGroup()]"
      @close-dialog="callGroupToDelete = null"
    >
      Are you sure you want to delete this call group:
      <strong>{{ callGroupToDeleteName }}</strong
      >?
    </ConfirmationDialog>
  </v-container>
</template>
...
const filterCallGroups = computed(() => {
  return CallGroups.value.filter((cg) => {
    return !cg.archived
  })
})
