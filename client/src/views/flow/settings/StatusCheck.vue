...
export default {
  ...
  methods: {
    async getStandardStatusReq(idx, statusPageUrl) {
      return statusPageUrl ? axios.get(statusPageUrl, {
        ...
        this.standardStatusPages[idx].incidents = resp?.data?.incidents || []
      }) : Promise.resolve()
    }
  },
  async created() {
    const requests = [this.standardStatusPages.filter(ssp => ssp.doStatusCheck).map((ssp, idx) => this.getStandardStatusReq(idx, ssp.incidentsUrl))]
