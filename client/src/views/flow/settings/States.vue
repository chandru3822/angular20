<template>
  <v-container id="states-settings">
    <v-row>
      <v-col class="shrink" cols="12">
        <v-toolbar flat class="app-toolbar">
          ...
          <v-toolbar-items>
            <a-btn
              ...
              color="primary"
              v-if="userCanAdd"
              @click=";[(addNew = !addNew), (selectedState = {})]"
        ...
        <v-card v-if="addNew" class="pa-5 mb-2">
          ...
          <a-btn
            :disabled="!selectedState || !selectedState.id"
            color="primary"
        ...
        >
          ...
          <template #expanded-item="{ headers, item }">
            <td
              ...
              class="pa-4"
              :class="{ 'shaded-row': filterStates.indexOf(item) % 2 }"
            >
              <h3>Edit State</h3>
              <div class="mb-3">
                <a-text-field v-model="item.mapLatitude" label="Map Latitude" />
                <a-text-field
                  v-model="item.mapLongitude"
                  label="Map Longitude"
                />
                <a-text-field v-model="item.mapZoom" label="Map Zoom" />
                <label>Active:</label>
                <input class="ml-3" type="checkbox" v-model="item.active" />
              </div>
              <a-btn
                :disabled="
                  !item.mapLatitude || !item.mapLongitude || !item.mapZoom
                "
                color="primary"
                class="white--text mr-2"
    ...
    <ConfirmationDialog
      :open-dialog="!!stateToDelete"
      @confirm="deleteCompanyState"
      @close-dialog="stateToDelete = null"
    >
      Are you sure you want to delete this state:
      <strong>{{ stateToDeleteName }}</strong>
    </ConfirmationDialog>
  </v-container>
</template>
...
const filterStates = computed(() => {
  return orderBy(
    companyStates.value.filter((cs) => {
      return !cs.archived
    }),
    [(cs) => cs.state.toLowerCase()]
  )
})
...
const saveCompanyState = async (ol, isNew) => {
  ...
  try {
    ...
    appStore.showSnack(
      'ERROR',
      isNew ? 'Error Adding State' : 'Error Updating State'
    )
...
const deleteCompanyState = async () => {
  ...
  try {
    const { status } = await deleteRequest(
      `/state/companyState/${companyState.id}`
    )
