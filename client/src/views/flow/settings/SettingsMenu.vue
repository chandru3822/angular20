  ...
  <v-list :dense="vuetify.breakpoint.smAndUp" :color="vuetify.breakpoint.smAndDown ? 'grey lighten-4' : 'transparent'" :class="{'left-menu' : vuetify.breakpoint.smAndDown}">
    <template v-for="(item, index) in filteredMenu">
      ...
      <v-list-item
          ...
          :class="{
  'shaded-row':
    !item.path.includes('/settings/company/settings') && (
      item.pathMatch && item.pathMatchExclude
        ? route.path.includes(item.pathMatch) && !route.path.includes(item.pathMatchExclude)
        : item.pathMatch
