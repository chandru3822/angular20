...
INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created, migrated_created_date)
    (SELECT project.id,
            ...
            (SELECT id FROM flow.company_process_step_status_type WHERE process_step_status_type = 'Active'
                                                                    and company_id = (select id from flow.company where company_name = 'Direct Solar of America')) AS process_step_status_id,
...
(now() + interval '1 day')
     FROM flow.project
              INNER JOIN blueraven.deal d
                         ON project.id = d.id
     where appointment_date is null  AND originator_id = 12
     ...
     p as (
         ...
         where cfg.process_step_id = 2276
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id, timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1651 then coalesce(ad.start_date,d2.appointment_date)
                 when p.custom_field_group_assignment_id = 1924 then coalesce(ad.end_date,d2.appointment_date) else null end,
            ...
            case when p.custom_field_group_assignment_id = 1664 then blueraven.get_user_position_for_closer(d2.id::integer,((added_on  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')::date) else null end,
            case when p.custom_field_group_assignment_id = 11921 then d2.remote_appointment else null end,
...
(now() + interval '1 day')
     ...
     where appointment_date is not null
       ...
       and  originator_id = 12  and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')))returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 2601
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 2210 then (select id from flow.list_of_value where parent_id = 1553
                                                                                                       and name = d2.appointment_outcome)
         when p.custom_field_group_assignment_id = 17251 then (select plh.id from brs.proposal_log_history plh
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 9204 then (select aci.check_in_time
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
           -- case when p.custom_field_group_assignment_id = 23 then d2.props_double_check else null end,
            case when p.custom_field_group_assignment_id = 1859 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1950 then (select id from flow.list_of_value where parent_id = 2021
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6669 then (select id from flow.list_of_value where parent_id = 2099
                                                                                                         and name = d2.site_survey_type)
                 when p.custom_field_group_assignment_id = 3003 then (select up.id from blueraven.deal d
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
                                                         int_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1859 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 1950 then (select id from flow.list_of_value where parent_id = 2021
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6669 then (select id from flow.list_of_value where parent_id = 2099
                                                                                                         and name = d2.site_survey_type)
...
(now() + interval '1 day')
     ...
     where d.appointment_outcome = 'Pitched' and
       ...
       AND originator_id = 12 )returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 1067
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6175 then ((installation_agreement_request_submitted_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1703 then installation_agreement_signed_date
                 when p.custom_field_group_assignment_id = 9256 then financial_agreement_sent_date
                 when p.custom_field_group_assignment_id = 14222 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9269 then agreement_signed_date
                 else null end,
            case when p.custom_field_group_assignment_id = 1846 then (select up.id from blueraven.deal d
                 ...
                 when p.custom_field_group_assignment_id = 13598 then (select plh.id from brs.proposal_log_history plh
                                                                    inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                    where d2.id = d3.id and plh.project_id =d2.id limit 1)
...
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13845 then (select id from flow.list_of_value where parent_id = 1657
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9464 then (select id from flow.list_of_value where parent_id = 1865
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9724 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9802 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1885 then (select id from flow.list_of_value where parent_id = 1787
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id = 15236 then (select id from flow.list_of_value where parent_id = 2255
                                                                                                         and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id = 15262 then (select id from flow.list_of_value where parent_id = 2073
                                                                                                         and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id = 13884 then (select id from flow.list_of_value where parent_id = 2190
                                                                                                        and name::integer = d2.loan_term)
                 when p.custom_field_group_assignment_id = 15301 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id = 10062 then d2.led_lightbulb_quantity
                 when p.custom_field_group_assignment_id = 10075 then d2.smart_thermostat_quantity
                 when p.custom_field_group_assignment_id = 10205 then (select id from flow.list_of_value where parent_id = 1982
                                                                                                         and name = d2.base_product)
                 when p.custom_field_group_assignment_id = 13143 then (select id from flow.list_of_value where parent_id = 1735
                                                                                                          and name = d2.hoa_approval_needed)
                 else null end,
            case when p.custom_field_group_assignment_id = 13871 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2067 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2093 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9789 then d2.system_size
                 when p.custom_field_group_assignment_id = 14235 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2080 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13858 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2106 then d2.total_ancillary_cost_with_fees
                 else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
...
--                                                                                                         and name = d2.introduction_call)
            ...
            case when p.custom_field_group_assignment_id = 13871 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2067 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2093 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9879 then d2.system_size
                 when p.custom_field_group_assignment_id = 14235 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2080 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13858 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2106 then d2.total_ancillary_cost_with_fees
...
(now() + interval '1 day')
     ...
     where   originator_id = 12 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')) and
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                 --                                                                                       and name = d2.site_survey_type)
                -- else null end,
            case when p.custom_field_group_assignment_id = 11531 then d2.site_survey_scheduled_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
with process_step1 as (
    INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created,process_step_complete_date,migrated_created_date,migrated_org_id,migrated_start_time,migrated_end_time)
        (with deals as (
            ...
            where  originator_id = 12 and work_type_id = 7 and deleted is false
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1495 then P1.migrated_start_time
                 when p.custom_field_group_assignment_id = 1508 then p1.migrated_end_time else null end,
            case when p.custom_field_group_assignment_id = 1521 then p1.migrated_org_id
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                --                                                                                        and name = d2.site_survey_type)
                 else null end,
            case when p.custom_field_group_assignment_id = 11531 then d2.site_survey_scheduled_date else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1547 then ((site_survey_uploaded_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
(now() + interval '1 day')
     from flow.project p
              inner join blueraven.deal d on d.id = p.id
     where originator_id = 12 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]'))
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1729 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12883 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1690 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2028 then (select id from flow.list_of_value where parent_id = 1969
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2015 then (select id from flow.list_of_value where parent_id = 1800
                                                                                                        and name = d2.kwh_change)
                 when   p.custom_field_group_assignment_id = 1755 then (select up.id from blueraven.deal d
            ...
            case when p.custom_field_group_assignment_id = 1937 then (select array_agg(id)
                                                                    from flow.list_of_value
                                                                    where parent_id = 2086 and
                                                                            name in (
                                                                            select  * from
            ...
            case when p.custom_field_group_assignment_id = 1742 then ((final_design_created_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 13182 then d2.site_survey_quality_issue
                 when p.custom_field_group_assignment_id = 8827 then  d2.pv_watts_estimate else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case --when p.custom_field_group_assignment_id = 710 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9542 then financial_agreement_sent_date else null end,
            case when p.custom_field_group_assignment_id = 1599 then ((final_design_sent_to_customer_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 10010 then d2.system_size
                 when p.custom_field_group_assignment_id = 10036 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 10049 then d2.total_ancillary_cost_with_fees
                 when p.custom_field_group_assignment_id = 10114 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 10101 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 12363 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 12350 then d2.total_cash_down_payment
                 else null end,
            case when p.custom_field_group_assignment_id = 10127 then (select id from flow.list_of_value where parent_id = 1982
                                                                                                        and name = d2.base_product)
                 when p.custom_field_group_assignment_id =11401 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                                             where d2.id = d3.id and plh.project_id =d2.id limit 1)
                 when p.custom_field_group_assignment_id =10816 then (select id from flow.list_of_value where parent_id = 1787
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id =9828 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id =10023 then d2.first_year_production_estimate
                 when p.custom_field_group_assignment_id =12337 then (select id from flow.list_of_value where parent_id = 2073
                                                                                                        and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id =9997 then panel_watts
                 when p.custom_field_group_assignment_id =9984 then panel_quantity
                 when p.custom_field_group_assignment_id =12376 then (select id from flow.list_of_value where parent_id = 2190
                                                                                                        and name::integer = d2.loan_term)
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9969 then (select id from flow.list_of_value where parent_id = 1865
                 and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12467 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12688 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10140 then (select id from flow.list_of_value where parent_id = 2255
                                                                                                        and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id =17273 then (select design_nbr
...
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9971 then (select id from flow.list_of_value where parent_id = 1865
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12467 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12688 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10140 then (select id from flow.list_of_value where parent_id = 2255
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id =1106
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
                                                         date_value,
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6435 then final_design_signed_date
                 when p.custom_field_group_assignment_id = 15379 then agreement_signed_date
                 when p.custom_field_group_assignment_id = 16705 then countersign_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
