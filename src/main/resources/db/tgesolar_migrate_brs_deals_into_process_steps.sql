...
INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created, migrated_created_date)
    (SELECT project.id,
            ...
            (SELECT id FROM flow.company_process_step_status_type WHERE process_step_status_type = 'Active'
                                                                    and company_id = (select id from flow.company where company_name = 'TGE Solar')) AS process_step_status_id,
...
(now() + interval '1 day')
     FROM flow.project
              INNER JOIN blueraven.deal d
                         ON project.id = d.id
     where appointment_date is null  AND originator_id = 10
     ...
     p as (
         ...
         where cfg.process_step_id = 2274
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id, timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1649 then coalesce(ad.start_date,d2.appointment_date)
                 when p.custom_field_group_assignment_id = 1922 then coalesce(ad.end_date,d2.appointment_date) else null end,
            ...
            case when p.custom_field_group_assignment_id = 1662 then blueraven.get_user_position_for_closer(d2.id::integer,((added_on  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')::date) else null end,
            case when p.custom_field_group_assignment_id = 11919 then d2.remote_appointment else null end,
...
(now() + interval '1 day')
     ...
     where appointment_date is not null
       ...
       and  originator_id = 10  and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')))returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 2599
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 2208 then (select id from flow.list_of_value where parent_id = 1551
                                                                                                       and name = d2.appointment_outcome)
         when p.custom_field_group_assignment_id = 17249 then (select plh.id from brs.proposal_log_history plh
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 9202 then (select aci.check_in_time
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
           -- case when p.custom_field_group_assignment_id = 23 then d2.props_double_check else null end,
            case when p.custom_field_group_assignment_id = 1857 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1948 then (select id from flow.list_of_value where parent_id = 2019
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6667 then (select id from flow.list_of_value where parent_id = 2097
                                                                                                         and name = d2.site_survey_type)
                 when p.custom_field_group_assignment_id = 3001 then (select up.id from blueraven.deal d
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
                                                         int_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1857 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 1948 then (select id from flow.list_of_value where parent_id = 2019
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6667 then (select id from flow.list_of_value where parent_id = 2097
                                                                                                         and name = d2.site_survey_type)
...
(now() + interval '1 day')
     ...
     where d.appointment_outcome = 'Pitched' and
       ...
       AND originator_id = 10 )returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 1065
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6173 then ((installation_agreement_request_submitted_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1701 then installation_agreement_signed_date
                 when p.custom_field_group_assignment_id = 9254 then financial_agreement_sent_date
                 when p.custom_field_group_assignment_id = 14220 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9267 then agreement_signed_date
                 else null end,
            case when p.custom_field_group_assignment_id = 1844 then (select up.id from blueraven.deal d
                 ...
                 when p.custom_field_group_assignment_id = 13596 then (select plh.id from brs.proposal_log_history plh
                                                                    inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                    where d2.id = d3.id and plh.project_id =d2.id limit 1)
...
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13843 then (select id from flow.list_of_value where parent_id = 1655
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9462 then (select id from flow.list_of_value where parent_id = 1863
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9722 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9800 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1883 then (select id from flow.list_of_value where parent_id = 1785
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id = 15234 then (select id from flow.list_of_value where parent_id = 2253
                                                                                                         and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id = 15260 then (select id from flow.list_of_value where parent_id = 2071
                                                                                                         and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id = 13882 then (select id from flow.list_of_value where parent_id = 2188
                                                                                                        and name::integer = d2.loan_term)
                 when p.custom_field_group_assignment_id = 15299 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id = 10060 then d2.led_lightbulb_quantity
                 when p.custom_field_group_assignment_id = 10073 then d2.smart_thermostat_quantity
                 when p.custom_field_group_assignment_id = 10203 then (select id from flow.list_of_value where parent_id = 1980
                                                                                                         and name = d2.base_product)
                 when p.custom_field_group_assignment_id = 13141 then (select id from flow.list_of_value where parent_id = 1733
                                                                                                          and name = d2.hoa_approval_needed)
                 else null end,
            case when p.custom_field_group_assignment_id = 13869 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2065 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2091 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9787 then d2.system_size
                 when p.custom_field_group_assignment_id = 14233 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2078 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13856 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2104 then d2.total_ancillary_cost_with_fees
                 else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 1844 then (select up.id from blueraven.deal d
                ...
                --                  when p.custom_field_group_assignment_id = 14 then (select id from flow.list_of_value where parent_id = 140
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13843 then (select id from flow.list_of_value where parent_id = 1654
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9462 then (select id from flow.list_of_value where parent_id = 1862
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9722 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9800 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1883 then (select id from flow.list_of_value where parent_id = 1785
            ...
            case when p.custom_field_group_assignment_id = 13869 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2065 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2091 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9877 then d2.system_size
                 when p.custom_field_group_assignment_id = 14233 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2078 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13856 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2104 then d2.total_ancillary_cost_with_fees
...
(now() + interval '1 day')
     ...
     where   originator_id = 10 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')) and
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                 --                                                                                       and name = d2.site_survey_type)
                -- else null end,
            case when p.custom_field_group_assignment_id = 11529 then d2.site_survey_scheduled_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
with process_step1 as (
    INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created,process_step_complete_date,migrated_created_date,migrated_org_id,migrated_start_time,migrated_end_time)
        (with deals as (
            ...
            where  originator_id = 10 and work_type_id = 7 and deleted is false
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1493 then P1.migrated_start_time
                 when p.custom_field_group_assignment_id = 1506 then p1.migrated_end_time else null end,
            case when p.custom_field_group_assignment_id = 1519 then p1.migrated_org_id
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                --                                                                                        and name = d2.site_survey_type)
                 else null end,
            case when p.custom_field_group_assignment_id = 11529 then d2.site_survey_scheduled_date else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1545 then ((site_survey_uploaded_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
(now() + interval '1 day')
     from flow.project p
              inner join blueraven.deal d on d.id = p.id
     where originator_id = 10 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]'))
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
                                                         ...
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1727 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12881 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1688 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2026 then (select id from flow.list_of_value where parent_id = 1967
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2013 then (select id from flow.list_of_value where parent_id = 1798
                                                                                                        and name = d2.kwh_change)
                 when   p.custom_field_group_assignment_id = 1754 then (select up.id from blueraven.deal d
            ...
            case when p.custom_field_group_assignment_id = 1935 then (select array_agg(id)
                                                                    from flow.list_of_value
                                                                    where parent_id = 2084 and
                                                                            name in (
                                                                            select  * from
            ...
            case when p.custom_field_group_assignment_id = 1740 then ((final_design_created_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 13180 then d2.site_survey_quality_issue
                 when p.custom_field_group_assignment_id = 8825 then  d2.pv_watts_estimate else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
                                                         ...
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1728 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12881 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1688 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2026 then (select id from flow.list_of_value where parent_id = 1967
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2013 then (select id from flow.list_of_value where parent_id = 1798
                                                                                                        and name = d2.kwh_change)
                 when   p.custom_field_group_assignment_id = 1753 then (select up.id from blueraven.deal d
                                                                                            inner join blueraven.user u on u.first_name|| ' '||u.last_name = d.final_design_completed_by
                                                                                            inner join flow.user_position up on up.user_id = u.id
                                                                      where d.id = d2.id and up.position_id in (24,25,26) and d.final_design_completed_by is not null limit 1)
                 else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case --when p.custom_field_group_assignment_id = 710 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9540 then financial_agreement_sent_date else null end,
            case when p.custom_field_group_assignment_id = 1597 then ((final_design_sent_to_customer_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 10008 then d2.system_size
                 when p.custom_field_group_assignment_id = 10034 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 10047 then d2.total_ancillary_cost_with_fees
                 when p.custom_field_group_assignment_id = 10112 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 10099 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 12361 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 12348 then d2.total_cash_down_payment
                 else null end,
            case when p.custom_field_group_assignment_id = 10125 then (select id from flow.list_of_value where parent_id = 1980
                                                                                                        and name = d2.base_product)
                 when p.custom_field_group_assignment_id =11399 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                                             where d2.id = d3.id and plh.project_id =d2.id limit 1)
                 when p.custom_field_group_assignment_id =10814 then (select id from flow.list_of_value where parent_id = 1785
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id =9826 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id =10021 then d2.first_year_production_estimate
                 when p.custom_field_group_assignment_id =12335 then (select id from flow.list_of_value where parent_id = 2071
                                                                                                        and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id =9995 then panel_watts
                 when p.custom_field_group_assignment_id =9982 then panel_quantity
                 when p.custom_field_group_assignment_id =12374 then (select id from flow.list_of_value where parent_id = 2188
                                                                                                        and name::integer = d2.loan_term)
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9967 then (select id from flow.list_of_value where parent_id = 1863
                 and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12465 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12686 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10138 then (select id from flow.list_of_value where parent_id = 2253
                                                                                                        and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id =17279 then (select design_nbr
...
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9969 then (select id from flow.list_of_value where parent_id = 1863
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12465 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12686 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10138 then (select id from flow.list_of_value where parent_id = 2253
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id =1104
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
                                                         date_value,
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6433 then final_design_signed_date
                 when p.custom_field_group_assignment_id = 15377 then agreement_signed_date
                 when p.custom_field_group_assignment_id = 16703 then countersign_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
