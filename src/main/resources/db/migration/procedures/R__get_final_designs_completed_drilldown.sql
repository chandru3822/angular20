...
-- Custom fields needed for R__get_final_designs_completed_drilldown query --
    ...
    -- 'First Cash Payment Paid' p_object_type_id = 4, p_custom_field_id = 112, p_process_step_id = 57
	...
	RETURN QUERY select array_to_json(array_agg(row_to_json(sub_rows)))
        from (
            ...
            from flow.project p
                ...
                inner join lateral (select * from flow.get_value_for_custom_field(4, 333, p.id, 4, false) as system_size) system_size on true
                ...
                left join lateral (select * from flow.get_value_for_custom_field(1, 52, p.id, 0, false) as cancelled_date) cancelled_date on true
