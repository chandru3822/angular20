...
BEGIN
  ...
  if new.recipient_type_id = 1  then  --user
    ...
    values(new.user_id,new.message,coalesce(new.message_sent_by_user_id,new.user_id),coalesce(new.message_sent_by_user_id,new.user_id),true)
...
BEGIN
  ...
  if new.to_phone != '+18014480212'  then  --user
    ...
    values(v_user_id,case when length(new.body) > 0 then new.body when new.num_media > 0 then 'Customer sent Image' else '' end,false)
    on conflict (user_id) do update
    ...
    insert into flow.sms_cache (project_id, message_text,outbound_message)
    values(v_project_id,case when length(new.body) > 0 then new.body when new.num_media > 0 then 'Customer sent Image' else '' end,false)
    on conflict (project_id) do update
      set message_text = excluded.message_text;
  end if;
