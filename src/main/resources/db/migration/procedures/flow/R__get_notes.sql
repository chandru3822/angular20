...
CREATE OR REPLACE FUNCTION flow.get_notes(p_primary_id bigint, p_object_type_id bigint, p_company_id bigint)
  ...
  RETURNS TABLE
          (
            ...
            primary_id                  bigint,
            created_by_primary_position json,
            child_notes                 json
          )
AS
...
BEGIN
  ...
  case
    when p_object_type_id = 1 then RETURN QUERY
      select n.id::bigint,
             n.note,
             n.archived,
             n.parent_id::bigint,
             n.date_created,
             n.date_modified,
             n.created_by_id::bigint,
             concat(creator.first_name, ' ', creator.last_name)           as created_by,
             n.modified_by_id::bigint,
             pn.project_id::bigint                                        as primary_id,
             (SELECT row_to_json(pp)
              FROM (select distinct upv.user_position_id as id,
                    ...
                    from flow.user_positions_vw upv
                           inner join flow.user_position_hierarchy_vw uphv
                                      on uphv.user_id = upv.user_id and uphv.org_id = upv.org_id and
                                         uphv.position_id = upv.position_id
                    ...
                    limit 1) pp)                                          AS "createdByPrimaryPosition",
             coalesce((SELECT array_to_json(array_agg(row_to_json(childNotes)))
                       FROM (select n2.id,
                                    n2.note,
                                    n2.archived,
                                    ...
                                    (SELECT row_to_json(pp)
                                     FROM (select distinct upv.user_position_id as id,
                                           ...
                                           from flow.user_positions_vw upv
                                                  inner join flow.user_position_hierarchy_vw uphv
                                                             on uphv.user_id = upv.user_id and
                                                                uphv.org_id = upv.org_id and
                                                                uphv.position_id = upv.position_id
                             ...
                             order by n2.date_created) childNotes), '[]') AS "child_notes"
      from flow.note n
    ...
    when p_object_type_id = 2 then RETURN QUERY
      select n.id::bigint,
             n.note,
             n.archived,
             n.parent_id::bigint,
             ...
             concat(creator.first_name, ' ', creator.last_name)           as created_by,
             n.modified_by_id::bigint,
             cn.contact_id::bigint                                        as primary_id,
             (SELECT row_to_json(pp)
              FROM (select distinct upv.user_position_id as id,
                                    upv.primary_flag     as "primaryFlag",
                                    upv.position,
      ...
      order by n.date_created desc;
    when p_object_type_id = 4 then RETURN QUERY
      select n.id::bigint,
             n.note,
             n.archived,
             n.parent_id::bigint,
             ...
             concat(creator.first_name, ' ', creator.last_name)           as created_by,
             n.modified_by_id::bigint,
             pn.project_process_step_id::bigint                           as primary_id,
             (SELECT row_to_json(pp)
              FROM (select distinct upv.user_position_id as id,
                                    upv.primary_flag     as "primaryFlag",
                                    upv.position,
      ...
      order by n.date_created desc;
    when p_object_type_id = 3 then RETURN QUERY
      select n.id::bigint,
             n.note,
             n.archived,
             n.parent_id::bigint,
             ...
             concat(creator.first_name, ' ', creator.last_name)           as created_by,
             n.modified_by_id::bigint,
             un.user_id::bigint                                           as primary_id,
             (SELECT row_to_json(pp)
              FROM (select distinct upv.user_position_id as id,
                                    upv.primary_flag     as "primaryFlag",
                                    upv.position,
      ...
      order by n.date_created desc;
    when p_object_type_id = 5 then RETURN QUERY
      select n.id::bigint,
             n.note,
             n.archived,
             n.parent_id::bigint,
             ...
             concat(creator.first_name, ' ', creator.last_name)           as created_by,
             n.modified_by_id::bigint,
             un.org_id::bigint                                            as primary_id,
             (SELECT row_to_json(pp)
              FROM (select distinct upv.user_position_id as id,
                                    upv.primary_flag     as "primaryFlag",
                                    upv.position,
