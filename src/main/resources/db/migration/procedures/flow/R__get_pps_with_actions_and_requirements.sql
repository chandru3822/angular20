...
--         This query is where we'll join in the already run actions and  exclude those
                 ...
                 where pps.id = p_project_process_step_id
                   and psl.process_step_requirement_id is not null
                   and psa.archived is not true
                   and psa.trigger_automatically is true
                   and psa.action_type_id = 2
             )
             select ps.id                                         as "processStepId",
                    ...
                    coalesce((select array_to_json(array_agg(row_to_json(r)))
                              from (
                                       select id,
                                       ...
                                       from flow.get_project_process_step_requirements_with_values(
                                               p_project_process_step_id::integer, reqs.ids::integer[])
                                   ) r), '[]')                    as "autoTriggeredActionRequirements",
                    coalesce((select array_to_json(array_agg(row_to_json(a)))
                              from (
                                       select psa.id,
                                              ...
                                              case when (select count(1) from flow.project_process_step_action ppsa
                                                                                  inner join flow.process_step_action psa2 on ppsa.process_step_action_id = psa2.id
                                                         where ppsa.project_process_step_id = p_project_process_step_id
                                                           and psa2.id = psa.id
                                                        ) > 0 then true else false end as "alreadyTriggered",
                                              coalesce((
                                                           ...
                                                           FROM (
                                                                    ...
                                                                    FROM flow.process_step_logic psl
                                                                             ...
                                                                             left join flow.process_step_requirement psr
                                                                                       on psr.id = psl.process_step_requirement_id
                                                           ...
                                                           FROM (
                                                                    SELECT psacp.id,
                                                                           ...
                                                                           (
                                                                               ...
                                                                               where psa.process_step_id = ps.id
                                                                                 and psa.trigger_automatically is true
                                                                                 and psa.archived is not true
                                                                    ...
                                                                    FROM flow.process_step_action_child_process psacp
                                                                        ...
                                                                        inner join flow.company_process_step_status_type cpsst on psacp.company_process_step_status_type_id = cpsst.id
                                                                    WHERE psacp.process_step_action_id = psa.id
                                                                      and psacp.archived is not true
                                                                    order by psacp.display_order, ps.process_step_name
                                                                ) children),
                                                       '[]')                                              AS "processStepActionChildProcesses",
                                              coalesce((
                                                                           ...
                                                                           coalesce((
                                                                                        ...
                                                                                        FROM (
                                                                                                 ...
                                                                                                 from flow.db_function_param dfp
                                                                                                          left join flow.action_param_dynamic_value apdv
                                                                                                                    on apdv.db_function_param_id = dfp.id and
                                                                                                                       apdv.process_step_action_company_function_id = psacf.id
                                                                                                 where dfp.db_function_id = cf.db_function_id
                                                                                                   and dfp.parameter_type_id = 2
                                                                                                   and apdv.archived is not true
                                                                                             ) params),
                                                                                    '[]')                     AS "actionParamDynamicValues"
                                                       ...
                                                       '[]')                                              AS "processStepActionChildFunctions",
                                              coalesce((
                                                       ...
                                                       '[]')                                              AS "processStepActionLinks"
                                       from flow.process_step_action psa
                                                left join flow.company_process_step_status_type cpsst
                                                          on cpsst.id = psa.company_process_step_status_type_id
                                                left join flow.company_project_status_type cpst
                                                          on cpst.id = psa.company_project_status_type_id
             ...
             from reqs,
                  flow.project_process_step pps
                      ...
                      left join flow.process_step_process psp
                                on psp.process_step_id = pps.process_step_id and psp.company_process_id = cp.id
             where pps.id = p_project_process_step_id
         ) as sub_rows;
    RETURN v_json;
END;
