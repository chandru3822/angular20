...
CREATE OR REPLACE FUNCTION flow.get_project_process_step_requirements_with_values(p_project_process_step_id bigint, p_requirement_ids bigint[])
...
BEGIN
  RETURN QUERY
    with reqs as (select psr.id::bigint,
                         ...
                         coalesce((SELECT array_to_json(array_agg(row_to_json(lov)))
                                   FROM (select lv.id,
                                                lv.name
                                         from flow.list_of_value lv
                                         where lv.id = any (psr.list_of_value_ids)) lov), '[]')  AS list_of_values,
                         case
                         ...
                         df.function_name,
                         coalesce((SELECT array_to_json(array_agg(row_to_json(params)))
                                   FROM (select rpdv.id,
                                                rpdv.archived,
                                         ...
                                         from flow.db_function_param dfp
                                                left join flow.requirement_param_dynamic_value rpdv
                                                          on rpdv.db_function_param_id = dfp.id and
                                                             rpdv.process_step_requirement_id = psr.id
                                         where dfp.db_function_id = cfn.db_function_id
                                           and dfp.parameter_type_id = 2
                                           and rpdv.archived is not true) params),
                                  '[]')                                                          AS requirement_param_dynamic_values,
                         case when df.id is not null then
                                coalesce((SELECT array_to_json(array_agg(row_to_json(params)))
                                          FROM (select dfp.data_type_id       as "dataTypeId",
                                                       ...
                                                       rpdv.dynamic_value     as "dynamicValue"
                                                from flow.db_function_param dfp
                                                       left join flow.company_function_param cfp
                                                                 on cfp.db_function_param_id = dfp.id and cfp.archived is not true
                                                       ...
                                                       left join flow.project_process_step_custom_field_value ppscfv
                                                                 on ppscfv.project_process_step_id = pps.id and
                                                                    ...
                                                                    cfp.custom_field_group_assignment_id
                                                where dfp.db_function_id = df.id
                                                  and dfp.archived is not true
                                                order by dfp.display_order) params), '[]') else '[]' end             AS "companyFunctionParams",
                         case
                           when cf.list_of_value_id is not null then
                             coalesce((SELECT array_to_json(array_agg(row_to_json(lov)))
                                       FROM (select lv.id,
                                                    lv.name
                                             from flow.list_of_value lv
                                             where lv.parent_id = cf.list_of_value_id) lov), '[]')
                           when cf.company_system_list_id is not null then
                             coalesce((SELECT array_to_json(array_agg(row_to_json(lov)))
                                       FROM (select *
                                             from flow.get_system_list_options(cf.company_id, cf.company_system_list_id,
                                                                               true, cf.system_list_option_ids)) lov), '[]')
                  ...
                  from flow.process_step_requirement psr
                         ...
                         inner join flow.process_step_requirement_type psrt
                                    on psrt.id = psr.process_step_requirement_type_id
                         inner join flow.project_process_step pps on pps.process_step_id = psr.process_step_id and pps.id = p_project_process_step_id
                         inner join flow.project p on p.id = pps.project_id
                         left join flow.custom_field_group_assignment cfga on cfga.id = psr.custom_field_group_assignment_id and cfga.archived is false
    ...
    --this is dumb but i had to expand the column list cuz there was one value i didn't want to return as-is from the query above (project_process_step_id)
    select r.id,
           r.project_id,
