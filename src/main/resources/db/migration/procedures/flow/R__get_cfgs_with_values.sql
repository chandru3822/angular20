create or replace function flow.get_cfgs_with_values(p_object_type_id bigint, p_source_id bigint, p_secondary_id bigint, p_company_id bigint, p_is_system_admin boolean, p_user_position_ids bigint[], p_return_cfvs boolean, p_boolean_for_random_stuff boolean) returns json
    language plpgsql
as $$
DECLARE
v_json json;
...
create temp table cfvs
    (
        ...
        custom_field_group_assignment_read_only_allow    boolean,
        required                                   boolean,
        show_on_user_profile                       boolean,
        custom_field_group_assignment_hidden       boolean,
        custom_field_group_assignment_hidden_allow       boolean,
...
insert into cfvs(project_id, contact_id, custom_field_group_assignment_id, custom_field_group_id,
                 custom_field_group_assignment_read_only, custom_field_group_assignment_read_only_allow, required,
                 custom_field_group_assignment_hidden, custom_field_group_assignment_hidden_allow, ancillary_custom_field_group_assignment_id,
...
select case
       ...
       native_cfga.read_only_allow                                                       as custom_field_group_assignment_read_only_allow,
       native_cfga.required,
       native_cfga.hidden                                                          as custom_field_group_assignment_hidden,
       native_cfga.hidden_allow                                                          as custom_field_group_assignment_hidden_allow,
...
set id              = ancillary_ppscfv.id,
  ...
  from cfvs original_rows
                 inner join flow.project_process_step ancillary_process_step
on ancillary_process_step.process_step_id = original_rows.ancillary_process_step_id and
  ancillary_process_step.project_id = p_source_id and ancillary_process_step.main is true
  inner join flow.project_process_step_custom_field_value ancillary_ppscfv
  on ancillary_ppscfv.project_process_step_id = ancillary_process_step.id and
  ancillary_ppscfv.custom_field_group_assignment_id =
...
set id                                         = dvfc.id,
    ...
    date_value                                 = case
            ...
            int_array_value                            = case
                                                             when coalesce(df.data_type_id, cdt.data_type_id) = 7
                                                                 then (select *
                                                                       from flow.get_value_for_data_view_field(dvfc.id, new_data.project_id))::int[]
                                                             else null::int[] end
...
set has_list_values = case
    ...
    text_value      = case
                        when cfvs.custom_field_sql is not null and (cfvs.system_readonly OR cfvs.data_type_id = 12)
                          then
                          (select *
                           from flow.get_custom_sql_text_value(cfvs.custom_field_sql::text, false, p_company_id,
                                                               cfvs.project_id::bigint, case
                                                                                          when p_object_type_id = 4
                                                                                            then p_source_id
                                                                                          else null end))
                        else text_value end,
    list_of_values  = case
                        when ((cfvs.company_system_list_id is not null) OR
                              (cfvs.custom_field_sql is not null and cfvs.system_readonly is not true and
                               cfvs.data_type_id != 12)) then
...
FROM (select cfvs.id,
             ...
             cfvs.custom_field_group_assignment_read_only_allow    as "customFieldGroupAssignmentReadOnlyAllow",
             cfvs.required,
             cfvs.custom_field_group_assignment_hidden       as "customFieldGroupAssignmentHidden",
             cfvs.custom_field_group_assignment_hidden_allow       as "customFieldGroupAssignmentHiddenAllow",
      ...
      where cfvs.object_type_id = p_object_type_id::bigint
                and case
                        when p_object_type_id::bigint = 3 and p_boolean_for_random_stuff::boolean is true
