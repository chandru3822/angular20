CREATE OR REPLACE FUNCTION flow.set_user_cfv(p_user_id integer, p_current_user_id integer,p_cfga integer, p_value_to_save text, p_override_existing boolean default true)
    returns boolean AS
$BODY$
declare
...
--         raise notice 'hello world %', v_date_value_to_save;
    ...
    select ( select c.company_id
             from flow.contact c
             where c.id = p_user_id) = (select cot.company_id
    ...
    if v_request_is_valid is true then
        ...
        where cfv.contact_id = p_user_id
