CREATE OR REPLACE FUNCTION flow.set_contact_cfv(p_contact_id integer, p_user_id integer,p_cfga integer, p_value_to_save text, p_override_existing boolean default true)
    returns boolean AS
$BODY$
declare
...
--         raise notice 'hello world %', v_date_value_to_save;
    ...
    -- 9,System List
            if v_data_type_id = 1 then
                ...
                insert into flow.contact_custom_field_value(contact_id, custom_field_group_assignment_id, date_value, timestamp_value, boolean_value, text_value, numeric_value, int_value, int_array_value, created_by_id, date_created, modified_by_id, date_modified)
                values (p_contact_id, p_cfga, null, null, null, null, null, p_value_to_save::int, null, p_user_id, now(), p_user_id, now());
            elsif v_data_type_id = 7 then
                insert into flow.contact_custom_field_value(contact_id, custom_field_group_assignment_id, date_value, timestamp_value, boolean_value, text_value, numeric_value, int_value, int_array_value, created_by_id, date_created, modified_by_id, date_modified)
                values (p_contact_id, p_cfga, null, null, null, null, null, null, p_value_to_save::int[], p_user_id, now(), p_user_id, now());
            end if;
        else
