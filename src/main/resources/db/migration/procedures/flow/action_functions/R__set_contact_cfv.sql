drop function if exists flow.set_contact_cfv(p_contact_id bigint, p_user_id bigint,p_cfga bigint, p_value_to_save text, p_override_existing boolean, text);
CREATE OR REPLACE FUNCTION flow.set_contact_cfv(p_contact_id bigint, p_user_id bigint,p_cfga bigint, p_value_to_save text, p_override_existing boolean default false, p_secondary_value_to_save text default null)
    returns boolean AS
$BODY$
declare
...
--         raise notice 'hello world %', v_date_value_to_save;
    ...
    -- 9,System List
            if v_data_type_id = 1 then
                ...
                values (p_contact_id, p_cfga, null, null, null, p_value_to_save::text, null,null, null, p_user_id, now(), p_user_id, now(), case when v_data_type_id = 13 then coalesce(p_secondary_value_to_save,p_value_to_save)::text end);
