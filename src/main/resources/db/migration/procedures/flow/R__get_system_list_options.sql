DROP FUNCTION IF EXISTS flow.get_system_list_options(p_company_id bigint, p_company_system_list_id bigint, p_sub_options BOOLEAN, p_system_list_option_ids bigint[], p_int_value bigint);
CREATE OR REPLACE FUNCTION flow.get_system_list_options(p_company_id bigint, p_company_system_list_id bigint, p_sub_options BOOLEAN, p_system_list_option_ids bigint[] default null, p_int_value bigint default null)
...
--                             concat(upv.first_name,' ',upv.last_name::text) as name
            select distinct upv.user_position_id::bigint as id,
                    case when  ARRAY_LENGTH( p_system_list_option_ids::bigint[], 1 ) > 1
