...
BEGIN
  ...
  with dates as (
    select p_start_time + d.date                   as date,
           extract(dow from p_start_time + d.date) as day_of_week_id
    from generate_series(0, 99) as d(date)
    where p_start_time + d.date BETWEEN p_start_time and p_end_time - 1
  )
  select coalesce(rs.user_id, rs.org_id)                             as resource_id,
         (concat(d.date, ' ', rsa.start_time))::timestamp            as "start",
         case
           when (concat(d.date, ' ', rsa.start_time))::timestamp > (concat(d.date, ' ', rsa.end_time))::timestamp
             then (concat(d.date + 1, ' ', rsa.end_time))::timestamp
  ...
  where (ra.start_time between p_start_time and p_end_time
    ...
    OR ra.user_id = any (v_user_non_sluts))
  union
  ...
  where rs.archived is not true
    ...
    and up.start_date <= now() and
        (up.end_date is null or up.end_date >= now())
     OR rs.user_id = any ( v_user_sluts)
  into v_json;
  return v_json;
END;
$$
