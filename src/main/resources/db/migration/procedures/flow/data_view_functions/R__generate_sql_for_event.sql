...
BEGIN
  ...
  if p_in_event_details > 1 then
    p_sql = p_sql || $$ update_event_$$ || p_in_event_details ||
            $$ as (select distinct on (pps.project_id) pps.project_id as id,$$;
    p_text_array_tables = array_append(p_text_array_tables,
                                       ($$update_event_$$ || p_in_event_details || $$ ue_$$ || p_in_event_details)::character varying);
  ...
  for x in select dvcfc.field_to_update,
    ...
    loop
      ...
      p_sql = p_sql || $$ flow.get_unique_behavior_value($$ || x.unique_behavior_code || $$,
                                                              ppse.$$ || z.column_name ||
              $$::text,0::bigint, ppse.id,$$ || quote_literal('EVENT') || $$)::$$ || x.data_type ||
  ...
  p_sql = p_sql || $$ order by pps.project_id, ppse.date_created $$ || v_order || $$ ), $$;
