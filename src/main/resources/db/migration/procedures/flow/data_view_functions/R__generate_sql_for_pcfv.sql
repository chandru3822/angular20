...
BEGIN
      ...
      for x in select dvcfc.field_to_update,
        ...
        loop
          ...
          p_sql = p_sql || $$ flow.get_unique_behavior_value($$ || x.unique_behavior_code || $$, pcfv.$$
                    || flow.get_value_based_on_data_type(z.data_type_id) || $$::text,0::bigint, pcfv.id,$$|| quote_literal('PROJECT')||$$)::$$ || x.data_type ||
