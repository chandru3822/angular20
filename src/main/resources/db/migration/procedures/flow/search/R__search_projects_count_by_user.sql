DROP FUNCTION IF EXISTS flow.search_projects_count_by_user(varchar, int8, int8, bool, int8, varchar, _int8);

CREATE OR REPLACE FUNCTION flow.search_projects_count_by_user(p_searchterm character varying, p_company_id bigint, p_user_id bigint, p_is_parent boolean, p_company_project_status_type_id bigint DEFAULT NULL::bigint, p_search_column character varying DEFAULT NULL::character varying, p_partner_ids bigint[] DEFAULT ARRAY[]::bigint[])
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    result bigint;
BEGIN
    -- The parameter p_query_commissions is excluded because it is only relevant when returning detailed project data,
    -- especially the commissions_outstanding field. In a COUNT(*) function, we are not returning any project details,
    -- so the value of p_query_commissions has no effect on the number of matching projects.
