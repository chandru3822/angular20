DROP FUNCTION IF EXISTS flow.search_contacts_count_with_down_line(varchar, int8, _int8, bool, int8, _int8);

CREATE OR REPLACE FUNCTION flow.search_contacts_count_with_down_line(p_searchterm character varying, p_company_id bigint, p_object_category_ids bigint[], p_is_parent boolean, p_userid bigint, p_partner_ids bigint[] DEFAULT ARRAY[]::bigint[])
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  -- Get organization IDs for the user (active positions only)
