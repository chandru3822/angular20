...
BEGIN
  ...
  IF (TG_OP = 'INSERT') THEN
      insert into flow.work_queue_cycle(project_process_step_id, company_process_step_status_type_id,
                                        ...
                                        date_entered_queue,
                                        created_by_id)
        (select new.id, new.company_process_step_status_type_id, process.process_step_work_queue_type_process_step_status_type_id, now(), new.created_by_id
         from flow.get_process_step_work_queue_type_configs(new.process_step_id) process
         where
           ((new.company_process_step_status_type_id = process.company_process_status_type_id or
