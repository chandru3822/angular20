    ...
    when p_is_down_line is true then RETURN QUERY
      ...
      FROM (with project_ids as (with positions as (select up.org_id as parent_org_id, up.user_id as user_id
                                 ...
                                 from (select distinct p.id as project_ids
                                       ...
                                       where p3.archived is not true) as foo),
            ...
            where cpst.company_id = p_company_id
              and cpst.archived is false
              and not exists(select c2.company_project_status_type_id
                             from counts c2
                             where c2.company_project_status_type_id = cpst.id)
            order by 4) as limited_projects;
      ...
      FROM (with user_position_ids as (select array_agg(up.id) as user_position_ids
                 ...
                 project_ids as (select array_agg(project_ids) as project_ids
                                 from (select p.id as project_ids
                                       from flow.project p
                                       ...
                                       where p2.archived is not true) as foo),
                 counts as (select cpst.id                  as company_project_status_type_id,
                                   cpst.project_status_type as company_project_status,
                                   count(p.id)              as project_status_count,
                                   cpst.display_order
            ...
            where cpst.company_id = p_company_id
              ...
              and not exists(select c2.company_project_status_type_id
                             from counts c2
                             where c2.company_project_status_type_id = cpst.id)
            order by display_order) as limited_projects;
    end case;
END;
$function$
