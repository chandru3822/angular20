...
begin
    ...
    where st.id = st.parent_id
        and (st.search_to_phone = v_phone_number
          or st.search_from_phone = v_phone_number);

    if(v_thread_id is null) then
        insert into flow.sms_thread(message, from_phone, to_phone, recipient_type_id, message_sent_by_user_id, sent_to_user_id, sent_to_project_id, archived)
