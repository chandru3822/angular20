CREATE OR REPLACE FUNCTION brs.execute_data_view_field_configs(p_contains_children boolean,
                                                               p_value text,
                                                               p_dvfc_id integer,
                                                               p_id integer,
                                                               p_sql text,
      ...
      select dvcvc.field_to_update,
             flow.get_prepared_value(dvcvc.data_type_id,
                                     flow.get_unique_behavior_value(ubt.unique_behavior_type,
                                                                    p_value::integer)) as value
      from flow.data_view_child_view_config dvcvc
