                      ...
                      with pending_review as
                           ...
                           paid as
                             (select e.expense_budget_id, sum(e.expense_amount) amount
                              ...
                              group by e.expense_budget_id),
                           -- i think "other" was when they allowed for N/A budgets
--                            other as
--                              (select e.expense_budget_id, sum(e.expense_amount)amount
--                               from brs.expense e
--                               where e.expense_date between p_start_date and p_end_date
...
--                               group by e.expense_budget_id),
                           active_expense_budget AS (select *
                                                     from brs.expense_budget
                                                     where
                                                         (user_id, budget_type_id, date_created) in (SELECT eb.user_id,
                                                                               eb.budget_type_id,
                      ...
                      select eb.budget_type_id,
                             ...
                             sum(coalesce(pr.amount, 0))                              pending_review,
...
--                              sum(coalesce(o.amount,0)) other,
                             sum(eb.amount) - sum(coalesce(pr.amount, 0)) - sum(coalesce(pa.amount, 0)) -
                      ...
                      from active_expense_budget eb
                             ...
                             left outer join pending_review pr on eb.id = pr.expense_budget_id
