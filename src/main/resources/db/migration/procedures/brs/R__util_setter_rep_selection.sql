...
BEGIN
    ...
    case when (v_org_level_id < 6) OR (5 = any(v_current_position_ids)) OR (p_platform_user_id in (99999999)) then ---- Corporate and Regional
        RETURN QUERY
            ...
            from (
                ...
                from (
                    ...
                    from flow.user_positions_vw upv
                        ...
                        inner join flow.org o on upv.org_id = o.id
                    where upv.org_id is not null
                        and upv.org_id in (select (elem->>'office_id')::integer from json_array_elements(p_office_ids) elem)
