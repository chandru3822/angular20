...
declare
BEGIN
    RETURN QUERY select coalesce((select array_to_json(array_agg(row_to_json(brackets)))
                 from (
                          select tb.id                         as "tournamentBracketId",
                                 ...
                                 coalesce((select array_to_json(array_agg(row_to_json(tournament_rounds)))
                                  from (
                                           select tr.id,
                                                  ...
                                                  coalesce((select array_to_json(array_agg(row_to_json(tournament_matches)))
                                                   from (
                                                            select tm.id,
