                                   ...
                                   select id,
                                          ...
                                          (select count(1)
                                           ...
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date = (now() at time zone 'US/Mountain')::date
                                             ...
                                             and pd.source = any (p_brs_provided_source_ids)) as today_count,
                                          (select count(1)
                                           from brs.project_details pd
                                                    inner join flow.project p on p.id = pd.project_id
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date >=
                                             ...
                                             and pd.source is not null
                                             and pd.company_id = v_company_id
                                             and pd.source = any (p_brs_provided_source_ids)) as week_to_date_count,
                                          (select count(1)
                                           from brs.project_details pd
                                                    inner join flow.project p on p.id = pd.project_id
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date between p_custom_start_date and p_custom_end_date
                                             ...
                                             and pd.source is not null
                                             and pd.company_id = v_company_id
                                             and pd.source = any (p_brs_provided_source_ids)) as custom_date_range_count
                                   from brs.funnel
                                   ...
                                   select id,
                                          ...
                                          (select count(1)
                                           ...
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date = (now() at time zone 'US/Mountain')::date
                                             ...
                                             and pd.source is not null
                                             and pd.company_id = v_company_id
                                             and pd.source = any (p_self_gen_source_ids)) as today_count,
                                          (select count(1)
                                           from brs.project_details pd
                                                    inner join flow.project p on p.id = pd.project_id
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date >=
                                             ...
                                             and pd.company_id = v_company_id
                                             and pd.source is not null
                                             and pd.source = any (p_self_gen_source_ids)) as week_to_date_count,
                                          (select count(1)
                                           from brs.project_details pd
                                                    inner join flow.project p on p.id = pd.project_id
                                           where ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date between p_custom_start_date and p_custom_end_date
                                             ...
                                             and pd.source is not null
                                             and pd.company_id = v_company_id
                                             and pd.source = any (p_self_gen_source_ids)) as custom_date_range_count
                                   from brs.funnel
