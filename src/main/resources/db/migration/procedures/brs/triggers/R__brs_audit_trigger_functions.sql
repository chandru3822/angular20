...
-- ### AHJ NEW HOME ###
drop trigger if exists ahj_new_home_custom_field_value_audit_trg ON brs.feat_db_ahj_new_home_custom_field_value;

drop function if exists brs.ahj_new_home_audit();
CREATE OR REPLACE FUNCTION brs.ahj_new_home_audit()
    RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'INSERT') THEN
        insert into brs.feat_db_ahj_new_home_custom_field_value_audit(ahj_new_home_custom_field_value_id, old_value, new_value, date_modified, modified_by_id)
        values(new.id,null,case when new.date_value is not null then new.date_value::text
                                when new.timestamp_value is not null then new.timestamp_value::text
                                when new.text_value is not null then new.text_value
                                when new.numeric_value is not null then new.numeric_value::text
...
CREATE TRIGGER ahj_new_home_custom_field_value_audit_trg
    after INSERT or update or delete ON brs.feat_db_ahj_new_home_custom_field_value
    FOR EACH ROW EXECUTE PROCEDURE brs.ahj_new_home_audit();
