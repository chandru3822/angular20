...
BEGIN
  ...
  RETURN QUERY
    SELECT *,
           ...
           case
             when foo.cancelled_date is not null then
               0::numeric
             ELSE
               coalesce(foo.total_commissions, 0) end + case
                                                          when foo.cancelled_date is not null THEN
                                                            0::numeric
                                                          else coalesce(foo.total_overrides, 0) end -
    ...
    FROM (with details as (select foo.setter_user_id, cpa2.allocation
                           from (select pd3.setter_user_id, pc.commission_plan_id, count(1) as pitched_count
                                 from brs.project_details pd3
                                        inner join brs.project_commission pc on pc.project_id = pd3.project_id
                                        inner join brs.commission_plan cp
                                                   on cp.id = pc.commission_plan_id and cp.position_id = 4
                                 where pd3.setter_milestone_pay::date >= '2021-04-15'
                                   and pd3.setter_milestone_pay <= v_period_end_date
                                   and pd3.first_appointment_not_pitched_or_missed <= v_period_end_date
                                   and pd3.first_appointment_not_pitched_or_missed >= (v_period_end_date) - 7
                                 group by pd3.setter_user_id, pc.commission_plan_id) as foo
                                  inner join brs.commission_plan_allocation cpa2
    ...
    WHERE CASE
      ...
      AND CASE
            WHEN p_override_plan_id IS NOT NULL
              THEN foo.override_plan_id = p_override_plan_id
            ELSE 1 = 1 END
      AND CASE
    ...
    values ('Get Commission Account Details for Setters', 'p_payroll_id: ' || p_payroll_id ||
