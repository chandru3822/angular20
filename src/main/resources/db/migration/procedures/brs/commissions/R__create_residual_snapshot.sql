...
BEGIN
        ...
        FOR d IN
            SELECT (brs.get_residual_account_details(null)).*,r.selected_user_ids
            FROM brs.residual r
            ...
            LOOP
                ...
                VALUES ( p_residual_id,
                         ...
                         d.total,
                         case when d.user_id = any(d.selected_user_ids) then
                           true
                          else
                            false end,
                         now(),
          ...
          (select v_snapshot_id,
                  rqlf.project_id,
                  (select id from brs.user_residual_project_snapshot_type as urpst where urpst.user_residual_project_snapshot_code = 'LIFETIME_QUALIFIED_FDS'),
           ...
           from brs.get_residual_qualified_lifetime_fds(d.user_id) rqlf);
                ...
                insert into brs.user_residual_project_snapshot(user_residual_snapshot_id,
                  ...
                  (select v_snapshot_id,
                          rqlf.project_id,
                          (select id from brs.user_residual_project_snapshot_type as urpst where urpst.user_residual_project_snapshot_code = 'FDS_NOT_QUALIFIED_THIS_PERIOD'),
                          rqlf.final_design_complete_date,
                          rqlf.final_design_signed_date,
                          rqlf.utility_bill_verified_date,
                          rqlf.financial_agreement_signed_date,
                ...
                for x in select * from brs.get_residual_fds_qualified_this_period(d.user_id)
                loop
                    insert into brs.user_residual_project_snapshot(user_residual_snapshot_id,
                                                                   project_id,
                                                                   user_residual_project_snapshot_type_id,
                  ...
                  values(
                          v_snapshot_id,
                          x.project_id,
                          (select id from brs.user_residual_project_snapshot_type as urpst where urpst.user_residual_project_snapshot_code = 'FDS_QUALIFIED_THIS_PERIOD'),
                  ...
                  insert into brs.residual_project_qualified_date(project_id, qualified_date, date_created, date_modified, created_by_id, modified_by_id,residual_id)
                ...
                insert into brs.user_residual_project_snapshot(user_residual_snapshot_id,
                  ...
                  (select v_snapshot_id,
                          rqlf.project_id,
                          (select id from brs.user_residual_project_snapshot_type as urpst where urpst.user_residual_project_snapshot_code = 'CLAWBACKS'),
                          rqlf.amount,
                           now(),
                          p_updated_by_id,
                          now(),
                ...
                insert into brs.residual_project_qualified_date(project_id, qualified_date, date_created, date_modified, created_by_id, modified_by_id,residual_id,excluded_from_cancel)
                  (select a.project_id,now(),now(),now(),p_updated_by_id,p_updated_by_id,p_residual_id,true
                   from brs.get_residual_fds_qualified_this_period(d.user_id,true) a );
