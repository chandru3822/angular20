...
--   raise notice 'p_fifteenth_current_month %',p_fifteenth_current_month;
  ...
  return query
    ...
    from (select pd.project_id,
                 ...
                 pd.system_size,
                 case
                   when rpsa.source_id is not null then
                 ...
                 rp.is_system_size,
                 case
                   when p_total_lifetime_fdc > 0 then
                     (select *
                      from brs.get_residual_project_plan_total(rp.id,
                                                               p_closer_user_id,
                                                               pd.system_size,
          ...
          from brs.project_details pd
                 ...
                 left join brs.residual_plan_source_allocation rpsa
                           on rpsa.residual_plan_id = rp.id and rpsa.source_id = pd.source
          ...
          where pd.final_design_signed_date >= v_min_start_date
            ...
            and case
                  when p_include_cancel is false then
                    pd.cancelled_date is null
                  when p_include_cancel is true then
                    pd.cancelled_date is not null end
            ...
            and ((pd.financial_agreement_signed_date is not null and
                  ...
                  case
                    ...
                    else 1 = 1 end) or (rpoqd.override_qualified_date is not null))
            and not exists (select id
                            from brs.residual_project_qualified_date rpqd
                            where rpqd.project_id = pd.project_id
                            limit 1)) as foo
    where foo.final_design_signed_date <= p_end_of_previous_month
      and final_design_complete_date1 is not null
      and (case
             when foo.final_design_signed_date >= p_beginning_of_previous_month and
                  foo.final_design_signed_date <= p_end_of_previous_month then ----beginning of the qualifying month
               foo.final_design_complete_date1 >=
               p_beginning_of_previous_month and ----- previous month end of qualifying month
               foo.final_design_complete_date1 <=
               p_fifteenth_current_month end or -- beginning of the qualifying month to the current months 15th day
           case
             when foo.final_design_signed_date < p_beginning_of_previous_month then ----beginning of the month
               ...
               foo.final_design_complete_date1 <=
               p_fifteenth_current_month end); --15th of the qualifying month  to the 15th of the current month
END
$BODY$
  LANGUAGE plpgsql
  VOLATILE
