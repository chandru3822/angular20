...
begin
  ...
  from (select *,
        ...
        from (select u.first_name,
                     ...
                     (select count(1) from brs.get_residual_fds_qualified_this_period(u.id))               as qualified_this_period_fdc,
                     (select count(1)
                     ...
                     rp.total,
                     coalesce((select sum(ra.amount) from brs.residual_adjustment ra where  ra.user_id = u.id and ra.residual_id = r.id) ,0)          as adjustment_override
              from flow."user" u
                     ...
                     inner join flow.user_status_type ust on ust.id = cus.user_status_type_id and ust.company_id = 3 and ust.id in (9,14)
              ...
              where u.id in (2438980,
                             2433935,
                             2353957) and exists(select id from flow.user_position up2 where up2.user_id = u.id and
                                                                       up2.position_id in (1,2,3,517))) as foo
