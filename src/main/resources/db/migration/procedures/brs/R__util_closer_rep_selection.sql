CREATE OR REPLACE FUNCTION flow.util_closer_rep_selection(p_platform_user_id integer, p_region_ids json, p_office_ids json)
    RETURNS SETOF json
    LANGUAGE plpgsql
AS $function$
declare
...
BEGIN
    ...
    case when (v_org_level_id < 6) OR (v_current_position_ids && '{2}') or (p_platform_user_id = 99999999) then ---- Corporate and Regional
        RETURN QUERY
            select array_to_json(array_agg(row_to_json(sub_rows)))
            from (
                     select distinct user_id, name, active
                     from (
                              select upmv.user_id, concat(upmv.first_name, ' ', upmv.last_name) as name,
                                     case when (upmv.end_date is null or upmv.end_date >= (now() at time zone 'US/Mountain')::date) then true else false end as active
