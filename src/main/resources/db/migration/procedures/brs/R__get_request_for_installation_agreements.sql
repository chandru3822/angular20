    ...
    case when (p_view_all) THEN
        RETURN QUERY
            SELECT
                ...
                (case when (select count(*) from brs.proposal_log_history plh where plh.project_id = p.id and plh.sunpower_url is not null) > 0 then true else false end)
            FROM flow.project p
            ...
            WHERE (pd.cancelled_date is null or p_show_cancelled is true) AND pd.energized_date is null
