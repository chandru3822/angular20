...
BEGIN
  ...
  --there is a bug when a bad postal code is used like 11111. this should prevent that error
  if(v_round_robin_id is not null) then
      ...
      if p_remote is false and v_timezone is not null then
        create temp table excluded_appointments as (with user_ids as (select up.user_id,
                                                    ...
                                                    select ui.user_id,
                                                           ui.id,
                                                           ppse.start_time as start_time,
                                                           ppse.end_time   as end_time
                                                    from flow.project p
                                                           inner join flow.project_process_step pps
                                                                      on pps.project_id = p.id and
                                                                         pps.process_step_id = 1 and pps.archived is false
                                                           inner join flow.project_process_step_event ppse
                                                                      on pps.id = ppse.project_process_step_id and ppse.archived is false
                                                           inner join flow.company_event_status_type cest
                                                                      on ppse.company_event_status_type_id = cest.id
                                                           inner join flow.event_status_type est
                                                                      on cest.event_status_type_id = est.id and
                                                                         (est.id in (1, 2) or (p.id = p_project_id and cest.id = 24)) -- 24 = needs to be rescheduled, per judson we dont want a user to be available if they already have an appt in needs to be rescheduled
                                                           ...
                                                           inner join flow.process_step_status_type psst
                                                                      on psst.id = cpsst.process_step_status_type_id
                                                                        and psst.id in (1, 2)
                                                           inner join flow.company_project_status_type cpst3
                                                                      on cpst3.id = p.company_project_status_type_id
                                                           inner join flow.project_status_type pst4
                                                                      on cpst3.project_status_type_id = pst4.id
                                                                        and pst4.id in (1, 4)
