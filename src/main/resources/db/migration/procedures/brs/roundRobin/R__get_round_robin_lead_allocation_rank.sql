                                   ...
                                   from (select pczu.user_id,
                                                ...
                                                (select coalesce(
                                                                (select ((sum(appointment_count)::numeric) / v_number_of_weeks)
                                                                 ...
                                                                 where ca.schedule_date between v_start_date and v_end_date
                                                                   and ca.user_id = pczu.user_id),
                                                                0))                                     as average_availability,
                                                (select coalesce((select ((sum(appointment_count)::numeric))
                                                                  from brs.cached_appointment ca
                                                                  where ca.schedule_date between v_end_date and v_future_availability
                                                                    and ca.user_id = pczu.user_id),
                                         ...
                                         from flow.round_robin pcz
                                                  ...
                                                  INNER JOIN flow.user_status_type ust
                                                             on ust.id = cus.user_status_type_id and
                                                                ust.company_id = 3 AND ust.has_access is true
                                                  left join LATERAL brs.get_fdc_counts(pczu.user_id, up.id,
                                                                                       v_closer_gen_source_ids,
                                                                                       p_time_interval) fdc_counts
                                                            on true
                                         where pcz.id = p_round_robin_id
                                           ...
                                           and (up.end_date is null or
                                                up.end_date >= (now() at time zone 'US/Mountain')::date -
                                                               (p_time_interval || 'day')::interval)
