...
--              and (pd.closer_appointment_outcome is null or pd.closer_appointment_outcome != 4) --Cancelled
			...
			setter_funnel_1 as (
             ...
             and ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date = (now() at time zone 'US/Mountain')::date - 1
				     and pd.closer_appointment_start is not null
				     and ((pd.closer_appointment_start at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date) as yesterday_day_count,
				(select count(1)
				 from brs.project_details pd
             inner join flow.project p on p.id = pd.project_id
				     inner join flow.user_position up on (up.user_id = pd.setter_user_id and up.primary_flag is true and up.position_id = 4)
             ...
             inner join flow.project p on p.id = pd.project_id
				 ...
				 where pd.source = 525 --Setter Gen
				     ...
				     and ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date - 7
				     and pd.closer_appointment_start is not null
				     and ((pd.closer_appointment_start at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date) as prev_seven_day_count,
				(select count(1)
				 from brs.project_details pd
             inner join flow.project p on p.id = pd.project_id
				     inner join flow.user_position up on (up.user_id = pd.setter_user_id and up.primary_flag is true and up.position_id = 4)
             ...
             inner join flow.project p on p.id = pd.project_id
				 ...
				 where pd.source = 525 --Setter Gen
				     ...
				     and ((p.date_created at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date - 30
				     and pd.closer_appointment_start is not null
				     and ((pd.closer_appointment_start at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date) as prev_thirty_day_count,
				(select count(1)
				 from brs.project_details pd
             inner join flow.project p on p.id = pd.project_id
				     inner join flow.user_position up on (up.user_id = pd.setter_user_id and up.primary_flag is true and up.position_id = 4)
				 where pd.source = 525 --Setter Gen
				     ...
				     and pd.closer_appointment_start is not null
				     and ((pd.closer_appointment_start at time zone 'UTC') at time zone 'US/Mountain') :: date <= (now() at time zone 'US/Mountain')::date) as custom_date_range_count
				from brs.setter_funnel
				where id = 1
            ),
