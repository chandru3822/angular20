    ...
    case when lower(trim(p_brs_feat_db_table)) = 'design' then
        ...
        return (select coalesce( ( select lov.name = p_expected_value
            from brs.feat_db_ahj a
                 inner join brs.feat_db_ahj_design ad on ad.ahj_id = a.id
        ...
        where a.id = (select int_value
          ...
          and bcfga.id = p_brs_feat_db_cf_id ), false) as result);  -- this is the brs.custom_field_group_assignment.id for whichever field you want

    when lower(trim(p_brs_feat_db_table)) = 'permit' then
            -- gets the selected value for the ahj custom field
            return (select coalesce( ( select lov.name = p_expected_value
                                       from brs.feat_db_ahj a
                                                inner join brs.feat_db_ahj_permit ap on ap.ahj_id = a.id
                                                inner join brs.feat_db_ahj_permit_custom_field_value cfv on cfv.ahj_permit_id = ap.id
                                                inner join brs.custom_field_group_assignment bcfga on bcfga.id = cfv.custom_field_group_assignment_id
                                                inner join brs.custom_field bcf on bcf.id = bcfga.custom_field_id
                                                inner join brs.list_of_value lov on lov.id = cfv.int_value
                                       where a.id = (select int_value
    ...
    when lower(trim(p_brs_feat_db_table)) = 'inspection' then
            ...
            return (select coalesce( ( select lov.name = p_expected_value
                                       from brs.feat_db_ahj a
                                                ...
                                                inner join brs.feat_db_ahj_inspection_custom_field_value cfv on cfv.ahj_inspection_id = ai.id
                                                inner join brs.custom_field_group_assignment bcfga on bcfga.id = cfv.custom_field_group_assignment_id
                                                inner join brs.custom_field bcf on bcf.id = bcfga.custom_field_id
                                                inner join brs.list_of_value lov on lov.id = cfv.int_value
                                       where a.id = (select int_value
        ...
        when lower(trim(p_brs_feat_db_table)) = 'utility' then
            ...
            return (select coalesce( ( select lov.name = p_expected_value
                                         from brs.feat_db_utility au
                                         inner join brs.feat_db_utility_custom_field_value cfv on cfv.utility_id = au.id
                                         inner join brs.custom_field_group_assignment bcfga on bcfga.id = cfv.custom_field_group_assignment_id
                                         inner join brs.custom_field bcf on bcf.id = bcfga.custom_field_id
                                         inner join brs.list_of_value lov on lov.id = cfv.int_value
