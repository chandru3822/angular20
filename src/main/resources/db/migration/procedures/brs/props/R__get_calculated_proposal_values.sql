...
(
  ...
  monthly_cost_25_year_average_without_solar       money,
  monthly_cost_30_year_average_without_solar       money,
  monthly_cost_25_year_average_with_solar          money,
  remaining_monthly_electric_bill_25_year_average  money,
  remaining_monthly_electric_bill_30_year_average  money,
  total_cost_25_years                              money,
  total_cost_30_years                              money,
  total_savings_25_years                           money,
  total_savings_30_years                           money,
  monthly_solar_payment                            money,
  monthly_cost_today_without_solar                 money,
  monthly_cost_today_with_solar                    money,
  monthly_cost_today_avg_remaining_electrical_bill money,
  initial_monthly_payment_all_credits_to_loan      money,
  initial_monthly_payment_no_credits_to_loan       money,
  reamortized_monthly_payment_all_credits_to_loan  money,
  reamortized_monthly_payment_no_credits_to_loan   money,
  monthly_payment_all_credits_to_loan_after_term   money,
  monthly_payment_no_credits_to_loan_after_term    money,
  system_size                                      numeric,
  first_year_production_estimate                   bigint,
  total_system_cost                                money,
  referral_promotion                               money,
  total_loan_amount                                money,
  federal_tax_incentive_amount                     money,
  federal_tax_incentive_rate                       numeric,
  net_system_cost                                  money,
  utility_company                                  text,
  estimated_annual_energy_consumption_kwh          bigint,
  current_estimated_annual_utility_bill            money,
  current_estimated_cost_per_kwh                   money,
  utility_cost_escalator                           numeric,
  state_rebate_amount                              money,
  eto_rebate                                       money,
  csu_rebate                                       money,
  apr                                              numeric,
  loan_term                                        numeric,
  assumed_payment_by_month_18                      money,
  ...
  monthly_cost_30_year_average_with_solar          money,
  reamortized_payment_factor_without_itc_paydown   numeric,
  secondary_monthly_payment_no_credits_to_loan     money,
  panel_brand                                      varchar,
  ...
  other_adder_and_discount_amount                  money,
  adder_name                                       text,
  non_solar_cap                                    numeric,
  required_down_payment                            money,
  storage_type_id                                  bigint,
  storage_type                                     varchar,
  financier                                        varchar,
  financier_id                                     bigint,
  loan_price_storage                               money,
  cash_price_storage                               money,
...
--call first formula
  ...
  return query
    select v_proposal_id,
           ...
           cast(round(v_monthly_cost_25_year_average_without_solar, 2) as money),
           cast(round(v_monthly_cost_30_year_average_without_solar, 2) as money),
           cast(round(v_monthly_cost_25_year_average_with_solar, 2) as money),
           cast(round(v_remaining_monthly_electric_bill_25_year_average, 2) as money),
           cast(round(v_remaining_monthly_electric_bill_30_year_average, 2) as money),
           cast(round(v_total_cost_25_years, 2) as money),
           cast(round(v_total_cost_30_years, 2) as money),
           cast(round(v_total_savings_25_years, 2) as money),
           cast(round(v_total_savings_30_years, 2) as money),
           cast(round(v_monthly_solar_payment, 2) as money),
           cast(round(v_monthly_cost_today_without_solar, 2) as money),
           cast(round(v_monthly_cost_today_with_solar, 2) as money),
           cast(round(v_monthly_cost_today_avg_remaining_electrical_bill, 2) as money),
           cast(round(v_initial_monthly_payment_all_credits_to_loan, 2) as money),
           cast(round(v_initial_monthly_payment_no_credits_to_loan, 2) as money),
           cast(round(v_reamortized_monthly_payment_all_credits_to_loan, 2) as money),
           cast(round(v_reamortized_monthly_payment_no_credits_to_loan, 2) as money),
           cast(round(v_monthly_payment_all_credits_to_loan_after_term, 2) as money),
           cast(round(v_monthly_payment_no_credits_to_loan_after_term, 2) as money),
           v_system_size,
           v_first_year_production_estimate,
           cast(round(v_total_system_cost, 2) as money),
           cast(round(v_referral_promotion, 2) as money),
           cast(round(v_total_loan_amount, 2) as money),
           cast(round(v_federal_tax_incentive_amount, 2) as money),
           round(v_federal_tax_incentive_rate, 2) * 100,
           cast(round(v_net_system_cost, 2) as money),
           v_utility_company,
           v_estimated_annual_energy_consumption_kwh,
           cast(round(v_current_estimated_annual_utility_bill, 2) as money),
           cast(round(v_current_estimated_cost_per_kwh, 4) as money),
           round(v_utility_cost_escalator * 100, 2),
           cast(round(v_state_rebate_amount, 2) as money),
           cast(round(v_eto_rebate, 2) as money),
           cast(round(v_csu_rebate, 2) as money),
           round(v_apr * 100, 2),
           v_loan_term,
           cast(round(v_assumed_payment_by_month_18, 2) as money),
           ...
           v_panel_warranty,
           v_inverter_warranty,
           cast(round(v_down_payment_amount, 2) as money),
           v_version_id,
           ...
           cast(round(v_monthly_cost_30_year_average_with_solar, 2) as money),
           v_reamortized_payment_factor_without_itc_paydown,
           cast(round(v_secondary_monthly_payment_no_credits_to_loan, 2) as money),
           v_panel_brand,
           v_panel_quantity,
           v_inverter_brand_id,
           v_inverter_brand,
           ...
           cast(round(v_other_adder_and_discount_amount, 2) as money),
           v_adder,
           round(v_non_solar_cap, 2),
           cast(round(v_required_down_payment, 2) as money),
           v_storage_type_id,
           v_storage_type,
           v_financier,
           v_financier_id,
           cast(round(v_loan_price_storage, 2) as money),
           cast(round(v_cash_price_storage, 2) as money),
