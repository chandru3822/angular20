    ...
    for x in SELECT jsonb_array_elements::jsonb FROM jsonb_array_elements(p_aurora_design_summary -> 'arrays')
      loop
        ...
        v_value = coalesce(v_value, 0) +
                  least(p_rebate_cap_dollar_amount, (p_total_system_cost_before_rebates * p_rebate_cap_percentage_of_total),
                        case
                          when v_total_solar_resource_fraction > .8 then
                            v_panel_count * p_panel_wattage * p_rebate_rate end);
      end loop;
    if p_number_of_batteries > 0 then
      v_value = coalesce(v_value,0) + least(p_battery_rebate_cap_amount,(p_cash_price_storage * p_battery_rebate_cap_percent_of_total ),
