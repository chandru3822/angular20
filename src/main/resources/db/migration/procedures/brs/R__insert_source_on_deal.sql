...
BEGIN
    ...
    where c.id = v_contact_id and cf.company_id = v_company_id and field_name = 'Lead Source'
    ...
    if v_source is not null then
        ...
        from  flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.custom_field cf on cf.id = cfga.custom_field_id and field_name = 'Lead Source' and cf.company_id = v_company_id
                  inner join flow.list_of_value lov on lov.id = cf.list_of_value_id  and lov.name = 'Lead Source'
                  inner join flow.list_of_value lov1 on lov1.parent_id = lov.id
        where lov1.name = v_source
          and cfga.archived is false and cfg.archived is false and cf.archived is false;

        if v_custom_field_group_assignment_id is not null and v_int_value_id is not null then

            perform flow.set_pps_cfv(p_project_id,v_custom_field_group_assignment_id,v_int_value_id );
        else
            select cf.id
            ...
            values(v_company_feature_id,'Unable to assign Lead Source to Project '||p_project_id||'.',1,now(),2350555);
        end if;
    else
        select cf.id
        into v_company_feature_id
