      ...
      RETURN QUERY select array_to_json(array_agg(row_to_json(funnel_rows)))
                   from (
                          with project_data as(
                            select ppscfv1.int_value,ppse.start_time,coalesce(ppscfv2.timestamp_value, ppsea.date_created) as checked_in_time,o.id as org_id,up.user_id
...
--                                    )
        ...
        RETURN QUERY select array_to_json(array_agg(row_to_json(funnel_rows)))
                     from (
                              ...
                              from (
                                       with project_data as(
                                           select ppscfv1.int_value,ppse.start_time,coalesce(ppscfv2.timestamp_value, ppsea.date_created) as checked_in_time
                                           from flow.project_process_step pps
