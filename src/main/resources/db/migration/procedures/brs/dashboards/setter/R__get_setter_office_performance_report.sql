...
DECLARE
  ...
  v_sales_org_id bigint;
BEGIN

  select coalesce(u.sales_org_id, u.org_id), ol.level, u.sales_org_id
  into v_office_id,v_org_level_id,v_sales_org_id
  from flow.user_position u
         inner join flow.org o on o.id = u.org_id
         inner join flow.org_type ot on ot.id = o.org_type_id
         inner join flow.org_level ol on ol.id = ot.org_level_id and ol.company_id = 3
  ...
  if v_org_level_id < 7 and v_sales_org_id is null then
    return query
      SELECT null::bigint     AS total_appointments,
             null::bigint     AS total_pitches,
             null::bigint     AS pitch_percentage,
             null::bigint AS setter_office_to_beat_id,
             null::text   AS setter_office_to_beat_name,
             null::bigint     AS pitches_to_go,
             null::text   AS current_office_rank;
  else

    return query
      with data as (select coalesce(rpt.total_appointments, 0) as total_appointments,
                    ...
                    from (select (select count(1)::bigint
                                  from brs.project_details pd
                                         ...
                                         inner join flow.org o2 on
                                    (o2.id = up2.org_id or o2.id = coalesce(up2.sales_org_id, 0::bigint))
                           ...
                           join lateral brs.get_setter_office_to_beat(v_office_id, p_start_date, p_end_date) sotb
                                on true)
      SELECT d.total_appointments         AS total_appointments,
