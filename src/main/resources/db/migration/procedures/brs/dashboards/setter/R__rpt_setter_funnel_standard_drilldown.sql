...
drop function if exists brs.rpt_setter_funnel_standard_drilldown(p_start_date date,
                                                                 p_end_date date,
                                                                 p_funnel_id bigint,
                                                                 p_user_position_ids bigint[],
                                                                 p_org_ids bigint[],
                                                                 p_hide_inactive boolean);
CREATE OR REPLACE FUNCTION brs.rpt_setter_funnel_standard_drilldown(p_start_date date,
                                                                    p_end_date date,
                                                                    p_funnel_id bigint,
                                                                    p_user_position_ids bigint[],
                                                                    p_org_ids bigint[],
                     ...
                     where pd.company_id = 3
                       ...
                       and case
                             when p_hide_inactive is true then
                               cus.user_status_type_id in (9, 11, 14)
                             else true end
                       and case
...
--                                         else 1 = 1 end)
                       ...
                       and case
                             ...
                             when p_funnel_id = 29 then
                               (pd.first_appointment_pitched at time zone 'UTC' at time zone 'US/Mountain')::date between p_start_date and p_end_date
