drop function if exists brs.util_setter_district_selection(p_platform_user_id bigint, p_area_ids json, p_region_ids json);
  CREATE OR REPLACE FUNCTION brs.util_setter_district_selection(p_platform_user_id bigint, p_area_ids json, p_region_ids json)
    RETURNS SETOF json
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
  ...
  -- org_level_id of 6 = District
  case when (v_org_level_id < 6) then
    RETURN QUERY
      ...
      from (
             ...
             from flow.user_positions_vw upv
                    inner join flow.org o on o.id = upv.org_id and o.org_type_id = 122
--                     inner join flow.org_type ot on o.org_type_id = ot.id and ot.id = 122

                    inner join flow.org o2 on o2.id = o.parent_org_id and o2.org_type_id = 4 -- region
--                     inner join flow.org_type ot1 on ot1.id = o2.org_type_id and ot1.id = 4 --region
                    inner join flow.org o3 on o3.id = o2.parent_org_id and o3.org_type_id = 20 -- area
--                     inner join flow.org_type ot2 on ot2.id = o3.org_type_id and ot2.id = 20 --area
...
--                     inner join flow.org_type ot1 on ot1.id = o2.org_type_id and ot1.id = 2 --region
             ...
             where upv.org_id is not null
               ...
               and  case when p_area_ids::text != '[]'::text then
                             o2.parent_org_id in (SELECT (elem ->> 'area_id') :: bigint
                                                  FROM json_array_elements(p_area_ids) elem)
                         else 1= 1 end
               and case when p_region_ids::text != '[]'::text then
                            o2.id in (SELECT (elem ->> 'region_id') :: bigint
...
values ('Setter District Selection', 'p_platform_user_id: ' || p_platform_user_id ||
