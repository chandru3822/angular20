drop function if exists brs.util_closer_district_selection(p_platform_user_id bigint, p_area_ids json, p_region_ids json, p_permission_override boolean );
CREATE OR REPLACE FUNCTION brs.util_closer_district_selection(p_platform_user_id bigint, p_area_ids json, p_region_ids json, p_permission_override boolean DEFAULT false)
	  RETURNS SETOF json
    LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- org_level_id of 6 = District
	  case when (p_permission_override) OR (v_org_level_id < 6) then
	      RETURN QUERY
...
--                     inner join flow.org_type ot on o.org_type_id = ot.id and ot.id = 117
                    ...
                    inner join flow.org o2 on o2.id = o.parent_org_id and o2.org_type_id = 2 -- region
--                     inner join flow.org_type ot1 on ot1.id = o2.org_type_id and ot1.id = 2 --region
                    inner join flow.org o3 on o3.id = o2.parent_org_id and o3.org_type_id = 21 -- area
--                     inner join flow.org_type ot2 on ot2.id = o3.org_type_id and ot2.id = 21 --area
...
--                     inner join flow.org_type ot1 on ot1.id = o2.org_type_id and ot1.id = 2 --region
    ...
    values ('Closer District Selection', 'p_platform_user_id: ' || p_platform_user_id ||
