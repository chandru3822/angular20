    ...
    with all_project_ids as (
        ...
        group by pay.id
         ...
         users AS (SELECT opru1.user_id                                                               AS user_id,
                          ...
                          coalesce(brs.get_total_overrides(p.id, a.v_all_projects, opru1.user_id), 0) as total_overrides
                   FROM brs.override_plan_receiving_user opru1
                            inner join brs.override_plan op1 on op1.id = opru1.override_plan_id
         ...
         commission_users AS (
             SELECT array_agg(DISTINCT p.id)                                                                   AS project_ids,
                    ...
                    coalesce(brs.get_ledger_totals(pay.id, array_agg(DISTINCT p.id), 1),
                             0)                                                                                as ledger_totals,
                    (select (select coalesce(sum(total), 0)
                             from (SELECT case
                                              when pd1.cancelled_date is not null then
                                                  0::numeric
                                              else coalesce(round(pd1.system_size::numeric * sum(opru.m1_allocation), 2),
                                                            0) end total
                                   FROM flow.project p1
                    ...
                    coalesce(brs.get_total_overrides(pay.id, a.v_all_projects, pd.closer_user_id),
