...
--   WITH milestones AS (
--       SELECT *
--       FROM brs.override_plan_milestone_allocation
--       WHERE override_plan_id = p_override_plan
--   )
--   INSERT INTO brs.override_plan_milestone_allocation
--   (override_plan_id, milestone_query_condition_id, allocation)
--     SELECT
--       _cloneId,
--       ms.milestone_query_condition_id,
--       ms.allocation
--     FROM milestones ms;
...
IF p_user_to_add IS NOT NULL THEN
  INSERT INTO brs
      .override_plan_receiving_user
  (override_plan_id, user_id)
  VALUES (_cloneId, p_user_to_add)
