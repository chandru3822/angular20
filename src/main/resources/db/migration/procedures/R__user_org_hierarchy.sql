...
p_org_id integer)
  RETURNS TABLE(org_id integer ,parent_org_id integer,org_name text,position_level integer, org_level_id integer) AS
$BODY$
declare
...
BEGIN
    return query
        WITH RECURSIVE subordinates(org_id,parent_org_id,org_name,position_level) AS (
            select o.id as org_id,o.parent_org_id,o.org_name,0, ot.org_level_id
            from flow.org o
                     inner join flow.org_type ot on ot.id = o.org_type_id
            where o.id = p_org_id
            UNION
            select o.id as org_id,o.parent_org_id,o.org_name,s.position_level + 1, ot.org_level_id
            from flow.org o
                     inner join flow.org_type ot on ot.id = o.org_type_id
                     INNER JOIN subordinates s ON s.parent_org_id = o.id
        ) SELECT
              s1.org_id,s1.parent_org_id,s1.org_name::text,s1.position_level,s1.org_level_id
        FROM
            subordinates s1;
END
$BODY$
