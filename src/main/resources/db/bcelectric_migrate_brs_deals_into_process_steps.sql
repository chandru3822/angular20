...
INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created, migrated_created_date)
    (SELECT project.id,
            ...
            (SELECT id FROM flow.company_process_step_status_type WHERE process_step_status_type = 'Active'
                                                                    and company_id = (select id from flow.company where company_name = 'B+C Electric')) AS process_step_status_id,
...
(now() + interval '1 day')
     FROM flow.project
              INNER JOIN blueraven.deal d
                         ON project.id = d.id
     where appointment_date is null  AND originator_id = 4
     ...
     p as (
         ...
         where cfg.process_step_id = 2268
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id, timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1643 then coalesce(ad.start_date,d2.appointment_date)
                 when p.custom_field_group_assignment_id = 1916 then coalesce(ad.end_date,d2.appointment_date) else null end,
            ...
            case when p.custom_field_group_assignment_id = 1656 then blueraven.get_user_position_for_closer(d2.id::integer,((added_on  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')::date) else null end,
            case when p.custom_field_group_assignment_id = 11913 then d2.remote_appointment else null end,
...
(now() + interval '1 day')
     ...
     where appointment_date is not null
       ...
       and  originator_id = 4  and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')))returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 2593
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 2202 then (select id from flow.list_of_value where parent_id = 1545
                                                                                                       and name = d2.appointment_outcome)
         when p.custom_field_group_assignment_id = 17246 then (select plh.id from brs.proposal_log_history plh
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 9196 then (select aci.check_in_time
            ...
            case when p.custom_field_group_assignment_id = 2202 then (select id from flow.list_of_value where parent_id = 1545
                                                                                                       and name = d2.appointment_outcome)
                 when p.custom_field_group_assignment_id = 17243 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                      where d2.id = d3.id and plh.project_id =d2.id limit 1)
                else null end,
            now(),now(),2350555,2350555
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
           -- case when p.custom_field_group_assignment_id = 23 then d2.props_double_check else null end,
            case when p.custom_field_group_assignment_id = 1851 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1942 then (select id from flow.list_of_value where parent_id = 2013
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6661 then (select id from flow.list_of_value where parent_id = 2091
                                                                                                         and name = d2.site_survey_type)
                 when p.custom_field_group_assignment_id = 2995 then (select up.id from blueraven.deal d
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
                                                         int_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1851 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 1942 then (select id from flow.list_of_value where parent_id = 2013
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6661 then (select id from flow.list_of_value where parent_id = 2091
                                                                                                         and name = d2.site_survey_type)
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 1059
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6167 then ((installation_agreement_request_submitted_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1695 then installation_agreement_signed_date
                 when p.custom_field_group_assignment_id = 9248 then financial_agreement_sent_date
                 when p.custom_field_group_assignment_id = 14214 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9261 then agreement_signed_date
                 else null end,
            case when p.custom_field_group_assignment_id = 1838 then (select up.id from blueraven.deal d
                 ...
                 when p.custom_field_group_assignment_id = 13590 then (select plh.id from brs.proposal_log_history plh
                                                                    inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                    where d2.id = d3.id and plh.project_id =d2.id limit 1)
...
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13837 then (select id from flow.list_of_value where parent_id = 1649
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9456 then (select id from flow.list_of_value where parent_id = 1857
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9716 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9794 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1877 then (select id from flow.list_of_value where parent_id = 1779
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id = 15228 then (select id from flow.list_of_value where parent_id = 2247
                                                                                                         and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id = 15254 then (select id from flow.list_of_value where parent_id = 2065
                                                                                                         and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id = 13876 then (select id from flow.list_of_value where parent_id = 2182
                                                                                                        and name::integer = d2.loan_term)
                 when p.custom_field_group_assignment_id = 15293 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id = 10054 then d2.led_lightbulb_quantity
                 when p.custom_field_group_assignment_id = 10067 then d2.smart_thermostat_quantity
                 when p.custom_field_group_assignment_id = 10197 then (select id from flow.list_of_value where parent_id = 1974
                                                                                                         and name = d2.base_product)
                 when p.custom_field_group_assignment_id = 13135 then (select id from flow.list_of_value where parent_id = 1727
                                                                                                          and name = d2.hoa_approval_needed)
                 else null end,
            case when p.custom_field_group_assignment_id = 13863 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2059 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2085 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9781 then d2.system_size
                 when p.custom_field_group_assignment_id = 14227 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2072 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13850 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2098 then d2.total_ancillary_cost_with_fees
                 else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
...
--                                                                                                         and name = d2.introduction_call)
            ...
            case when p.custom_field_group_assignment_id = 13863 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2059 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2085 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9871 then d2.system_size
                 when p.custom_field_group_assignment_id = 14227 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2072 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13850 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2098 then d2.total_ancillary_cost_with_fees
...
(now() + interval '1 day')
     ...
     where   originator_id = 4 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')) and
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                 --                                                                                       and name = d2.site_survey_type)
                -- else null end,
            case when p.custom_field_group_assignment_id = 11523 then d2.site_survey_scheduled_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
with process_step1 as (
    INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created,process_step_complete_date,migrated_created_date,migrated_org_id,migrated_start_time,migrated_end_time)
        (with deals as (
            ...
            where  originator_id = 4 and work_type_id = 7 and deleted is false
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1487 then P1.migrated_start_time
                 when p.custom_field_group_assignment_id = 1500 then p1.migrated_end_time else null end,
            case when p.custom_field_group_assignment_id = 1513 then p1.migrated_org_id
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                --                                                                                        and name = d2.site_survey_type)
                 else null end,
            case when p.custom_field_group_assignment_id = 11523 then d2.site_survey_scheduled_date else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1539 then ((site_survey_uploaded_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
(now() + interval '1 day')
     from flow.project p
              inner join blueraven.deal d on d.id = p.id
     where originator_id = 4 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]'))
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
                                                         ...
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1721 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12875 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1682 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2020 then (select id from flow.list_of_value where parent_id = 1961
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2007 then (select id from flow.list_of_value where parent_id = 1792
                                                                                                        and name = d2.kwh_change)
                 when   p.custom_field_group_assignment_id = 1747 then (select up.id from blueraven.deal d
            ...
            case when p.custom_field_group_assignment_id = 1929 then (select array_agg(id)
                                                                    from flow.list_of_value
                                                                    where parent_id = 2078 and
                                                                            name in (
                                                                            select  * from
            ...
            case when p.custom_field_group_assignment_id = 1734 then ((final_design_created_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 13174 then d2.site_survey_quality_issue
                 when p.custom_field_group_assignment_id = 8819 then  d2.pv_watts_estimate else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case --when p.custom_field_group_assignment_id = 710 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9534 then financial_agreement_sent_date else null end,
            case when p.custom_field_group_assignment_id = 1591 then ((final_design_sent_to_customer_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 10002 then d2.system_size
                 when p.custom_field_group_assignment_id = 10028 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 10041 then d2.total_ancillary_cost_with_fees
                 when p.custom_field_group_assignment_id = 10106 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 10093 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 12355 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 12342 then d2.total_cash_down_payment
                 else null end,
            case when p.custom_field_group_assignment_id =10119 then (select id from flow.list_of_value where parent_id = 1974
                                                                                                        and name = d2.base_product)
                 when p.custom_field_group_assignment_id =11393 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                                             where d2.id = d3.id and plh.project_id =d2.id limit 1)
                 when p.custom_field_group_assignment_id =10808 then (select id from flow.list_of_value where parent_id = 1779
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id =9820 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id =10015 then d2.first_year_production_estimate
                 when p.custom_field_group_assignment_id =12329 then (select id from flow.list_of_value where parent_id = 2065
                                                                                                        and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id =9989 then panel_watts
                 when p.custom_field_group_assignment_id =9976 then panel_quantity
                 when p.custom_field_group_assignment_id =12368 then (select id from flow.list_of_value where parent_id = 2182
                                                                                                        and name::integer = d2.loan_term)
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9963 then (select id from flow.list_of_value where parent_id = 1857
                 and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12459 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12680 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10132 then (select id from flow.list_of_value where parent_id = 2247
                                                                                                        and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id =17419 then (select design_nbr
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id =1098
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
                                                         date_value,
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6427 then final_design_signed_date
                 when p.custom_field_group_assignment_id = 15371 then agreement_signed_date
                 when p.custom_field_group_assignment_id = 16697 then countersign_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
