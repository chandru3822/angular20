...
INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created, migrated_created_date)
    (SELECT project.id,
            ...
            (SELECT id FROM flow.company_process_step_status_type WHERE process_step_status_type = 'Active'
                                                                    and company_id = (select id from flow.company where company_name = 'Salient Solar')) AS process_step_status_id,
...
(now() + interval '1 day')
     FROM flow.project
              INNER JOIN blueraven.deal d
                         ON project.id = d.id
     where appointment_date is null  AND originator_id = 6
     ...
     p as (
         ...
         where cfg.process_step_id = 2270
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id, timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1645 then coalesce(ad.start_date,d2.appointment_date)
                 when p.custom_field_group_assignment_id = 1918 then coalesce(ad.end_date,d2.appointment_date) else null end,
            ...
            case when p.custom_field_group_assignment_id = 1658 then blueraven.get_user_position_for_closer(d2.id::integer,((added_on  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')::date) else null end,
            case when p.custom_field_group_assignment_id = 11915 then d2.remote_appointment else null end,
...
(now() + interval '1 day')
     ...
     where appointment_date is not null
       ...
       and  originator_id = 6  and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')))returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 2595
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 2204 then (select id from flow.list_of_value where parent_id = 1547
                                                                                                       and name = d2.appointment_outcome)
         when p.custom_field_group_assignment_id = 17245 then (select plh.id from brs.proposal_log_history plh
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 9198 then (select aci.check_in_time
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
           -- case when p.custom_field_group_assignment_id = 23 then d2.props_double_check else null end,
            case when p.custom_field_group_assignment_id = 1853 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1944 then (select id from flow.list_of_value where parent_id = 2015
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6663 then (select id from flow.list_of_value where parent_id = 2093
                                                                                                         and name = d2.site_survey_type)
                 when p.custom_field_group_assignment_id = 2997 then (select up.id from blueraven.deal d
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
                                                         int_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1853 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 1944 then (select id from flow.list_of_value where parent_id = 2015
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6663 then (select id from flow.list_of_value where parent_id = 2093
                                                                                                         and name = d2.site_survey_type)
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 1061
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6169 then ((installation_agreement_request_submitted_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1697 then installation_agreement_signed_date
                 when p.custom_field_group_assignment_id = 9250 then financial_agreement_sent_date
                 when p.custom_field_group_assignment_id = 14216 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9263 then agreement_signed_date
                 else null end,
            case when p.custom_field_group_assignment_id = 1840 then (select up.id from blueraven.deal d
                 ...
                 when p.custom_field_group_assignment_id = 13592 then (select plh.id from brs.proposal_log_history plh
                                                                    inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                    where d2.id = d3.id and plh.project_id =d2.id limit 1)
...
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13839 then (select id from flow.list_of_value where parent_id = 1651
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9458 then (select id from flow.list_of_value where parent_id = 1859
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9718 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9796 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1879 then (select id from flow.list_of_value where parent_id = 1781
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id = 15230 then (select id from flow.list_of_value where parent_id = 2249
                                                                                                         and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id = 15256 then (select id from flow.list_of_value where parent_id = 2067
                                                                                                         and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id = 13878 then (select id from flow.list_of_value where parent_id = 2184
                                                                                                        and name::integer = d2.loan_term)
                 when p.custom_field_group_assignment_id = 15295 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id = 10056 then d2.led_lightbulb_quantity
                 when p.custom_field_group_assignment_id = 10069 then d2.smart_thermostat_quantity
                 when p.custom_field_group_assignment_id = 10199 then (select id from flow.list_of_value where parent_id = 1976
                                                                                                         and name = d2.base_product)
                 when p.custom_field_group_assignment_id = 13137 then (select id from flow.list_of_value where parent_id = 1729
                                                                                                          and name = d2.hoa_approval_needed)
                 else null end,
            case when p.custom_field_group_assignment_id = 13865 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2061 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2087 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9783 then d2.system_size
                 when p.custom_field_group_assignment_id = 14229 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2074 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13852 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2100 then d2.total_ancillary_cost_with_fees
                 else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
...
--                                                                                                         and name = d2.introduction_call)
            ...
            case when p.custom_field_group_assignment_id = 13865 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2061 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2087 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9873 then d2.system_size
                 when p.custom_field_group_assignment_id = 14229 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2074 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13852 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2100 then d2.total_ancillary_cost_with_fees
...
(now() + interval '1 day')
     ...
     where   originator_id = 6 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')) and
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                 --                                                                                       and name = d2.site_survey_type)
                -- else null end,
            case when p.custom_field_group_assignment_id = 11525 then d2.site_survey_scheduled_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
with process_step1 as (
    INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created,process_step_complete_date,migrated_created_date,migrated_org_id,migrated_start_time,migrated_end_time)
        (with deals as (
            ...
            where  originator_id = 6 and work_type_id = 7 and deleted is false
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1489 then P1.migrated_start_time
                 when p.custom_field_group_assignment_id = 1502 then p1.migrated_end_time else null end,
            case when p.custom_field_group_assignment_id = 1515 then p1.migrated_org_id
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                --                                                                                        and name = d2.site_survey_type)
                 else null end,
            case when p.custom_field_group_assignment_id = 11525 then d2.site_survey_scheduled_date else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1541 then ((site_survey_uploaded_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
(now() + interval '1 day')
     from flow.project p
              inner join blueraven.deal d on d.id = p.id
     where originator_id = 6 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]'))
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
                                                         ...
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1723 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12877 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1684 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2022 then (select id from flow.list_of_value where parent_id = 1963
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2009 then (select id from flow.list_of_value where parent_id = 1794
                                                                                                        and name = d2.kwh_change)
                 when   p.custom_field_group_assignment_id = 1749 then (select up.id from blueraven.deal d
            ...
            case when p.custom_field_group_assignment_id = 1931 then (select array_agg(id)
                                                                    from flow.list_of_value
                                                                    where parent_id = 2080 and
                                                                            name in (
                                                                            select  * from
            ...
            case when p.custom_field_group_assignment_id = 1736 then ((final_design_created_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 13176 then d2.site_survey_quality_issue
                 when p.custom_field_group_assignment_id = 8821 then  d2.pv_watts_estimate else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case --when p.custom_field_group_assignment_id = 710 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9536 then financial_agreement_sent_date else null end,
            case when p.custom_field_group_assignment_id = 1593 then ((final_design_sent_to_customer_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 10004 then d2.system_size
                 when p.custom_field_group_assignment_id = 10030 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 10043 then d2.total_ancillary_cost_with_fees
                 when p.custom_field_group_assignment_id = 10108 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 10095 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 12357 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 12344 then d2.total_cash_down_payment
                 else null end,
            case when p.custom_field_group_assignment_id =10121 then (select id from flow.list_of_value where parent_id = 1976
                                                                                                        and name = d2.base_product)
                 when p.custom_field_group_assignment_id =11395 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                                             where d2.id = d3.id and plh.project_id =d2.id limit 1)
                 when p.custom_field_group_assignment_id =10810 then (select id from flow.list_of_value where parent_id = 1781
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id =9822 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id =10017 then d2.first_year_production_estimate
                 when p.custom_field_group_assignment_id =12331 then (select id from flow.list_of_value where parent_id = 2067
                                                                                                        and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id =9991 then panel_watts
                 when p.custom_field_group_assignment_id =9978 then panel_quantity
                 when p.custom_field_group_assignment_id =12370 then (select id from flow.list_of_value where parent_id = 2184
                                                                                                        and name::integer = d2.loan_term)
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9965 then (select id from flow.list_of_value where parent_id = 1859
                 and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12461 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12682 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10134 then (select id from flow.list_of_value where parent_id = 2249
                                                                                                        and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id =17276 then (select design_nbr
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id =1100
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
                                                         date_value,
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6429 then final_design_signed_date
                 when p.custom_field_group_assignment_id = 15373 then agreement_signed_date
                 when p.custom_field_group_assignment_id = 16699 then countersign_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
