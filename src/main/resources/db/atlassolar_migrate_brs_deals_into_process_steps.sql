...
INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created, migrated_created_date)
    (SELECT project.id,
            ...
            (SELECT id FROM flow.company_process_step_status_type WHERE process_step_status_type = 'Active'
                                                                    and company_id = (select id from flow.company where company_name = 'Atlas Solar Advisors')) AS process_step_status_id,
...
(now() + interval '1 day')
     FROM flow.project
              INNER JOIN blueraven.deal d
                         ON project.id = d.id
     where appointment_date is null  AND originator_id = 11
     ...
     p as (
         ...
         where cfg.process_step_id = 2275
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id, timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1650 then coalesce(ad.start_date,d2.appointment_date)
                 when p.custom_field_group_assignment_id = 1923 then coalesce(ad.end_date,d2.appointment_date) else null end,
            ...
            case when p.custom_field_group_assignment_id = 1663 then blueraven.get_user_position_for_closer(d2.id::integer,((added_on  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')::date) else null end,
            case when p.custom_field_group_assignment_id = 11920 then d2.remote_appointment else null end,
...
(now() + interval '1 day')
     ...
     where appointment_date is not null
       ...
       and  originator_id = 11  and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')))returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 2600
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            ...
            case when p.custom_field_group_assignment_id = 2209 then (select id from flow.list_of_value where parent_id = 1552
                                                                                                       and name = d2.appointment_outcome)
         when p.custom_field_group_assignment_id = 17250 then (select plh.id from brs.proposal_log_history plh
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 9203 then (select aci.check_in_time
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
           -- case when p.custom_field_group_assignment_id = 23 then d2.props_double_check else null end,
            case when p.custom_field_group_assignment_id = 1858 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1949 then (select id from flow.list_of_value where parent_id = 2020
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6668 then (select id from flow.list_of_value where parent_id = 2098
                                                                                                         and name = d2.site_survey_type)
                 when p.custom_field_group_assignment_id = 3002 then (select up.id from blueraven.deal d
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
                                                         int_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1858 then ((proposal_complete_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 1949 then (select id from flow.list_of_value where parent_id = 2020
                                                                                                        and name = d2.proposal_status)
                 when p.custom_field_group_assignment_id = 6668 then (select id from flow.list_of_value where parent_id = 2098
                                                                                                         and name = d2.site_survey_type)
...
(now() + interval '1 day')
     ...
     where d.appointment_outcome = 'Pitched' and
       ...
       AND originator_id = 11 )returning *),
     p as (
         select cfga.id as custom_field_group_assignment_id,cf.field_name,dt.data_type,dt.id as data_type_id
         from flow.custom_field_group_assignment cfga
                  inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id = 1066
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6174 then ((installation_agreement_request_submitted_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 else null end,
            case when p.custom_field_group_assignment_id = 1702 then installation_agreement_signed_date
                 when p.custom_field_group_assignment_id = 9255 then financial_agreement_sent_date
                 when p.custom_field_group_assignment_id = 14221 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9268 then agreement_signed_date
                 else null end,
            case when p.custom_field_group_assignment_id = 1845 then (select up.id from blueraven.deal d
...
--                                                                                                         and name = d2.introduction_call)
                 when p.custom_field_group_assignment_id = 13844 then (select id from flow.list_of_value where parent_id = 1656
                                                                                                        and name = d2.credit_check)
                 when p.custom_field_group_assignment_id = 9463 then (select id from flow.list_of_value where parent_id = 1864
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id = 9723 then d2.panel_quantity
                 when p.custom_field_group_assignment_id = 9801 then d2.panel_watts
                 when p.custom_field_group_assignment_id = 1884 then (select id from flow.list_of_value where parent_id = 1786
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id = 15235 then (select id from flow.list_of_value where parent_id = 2254
                                                                                                         and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id = 15261 then (select id from flow.list_of_value where parent_id = 2072
                                                                                                         and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id = 13883 then (select id from flow.list_of_value where parent_id = 2189
                                                                                                        and name::integer = d2.loan_term)
                 when p.custom_field_group_assignment_id = 15300 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id = 10061 then d2.led_lightbulb_quantity
                 when p.custom_field_group_assignment_id = 10074 then d2.smart_thermostat_quantity
                 when p.custom_field_group_assignment_id = 10204 then (select id from flow.list_of_value where parent_id = 1981
                                                                                                         and name = d2.base_product)
                 when p.custom_field_group_assignment_id = 13142 then (select id from flow.list_of_value where parent_id = 1734
                                                                                                          and name = d2.hoa_approval_needed)
                 else null end,
            case when p.custom_field_group_assignment_id = 13870 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2066 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2092 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9788 then d2.system_size
                 when p.custom_field_group_assignment_id = 14234 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2079 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13857 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2105 then d2.total_ancillary_cost_with_fees
                 else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
...
--                                                                                                         and name = d2.introduction_call)
            ...
            case when p.custom_field_group_assignment_id = 13870 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 2066 then d2.interest_rate
                 when p.custom_field_group_assignment_id = 2092 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 9878 then d2.system_size
                 when p.custom_field_group_assignment_id = 14234 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 2079 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 13857 then d2.total_cash_down_payment
                 when p.custom_field_group_assignment_id = 2105 then d2.total_ancillary_cost_with_fees
...
(now() + interval '1 day')
     ...
     where   originator_id = 11 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]')) and
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value, date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                 --                                                                                       and name = d2.site_survey_type)
                -- else null end,
            case when p.custom_field_group_assignment_id = 11530 then d2.site_survey_scheduled_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
with process_step1 as (
    INSERT INTO flow.project_process_step (project_id, process_step_id, company_process_step_status_type_id, created_by_id,date_created,process_step_complete_date,migrated_created_date,migrated_org_id,migrated_start_time,migrated_end_time)
        (with deals as (
            ...
            where  originator_id = 11 and work_type_id = 7 and deleted is false
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,timestamp_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1494 then P1.migrated_start_time
                 when p.custom_field_group_assignment_id = 1507 then p1.migrated_end_time else null end,
            case when p.custom_field_group_assignment_id = 1520 then p1.migrated_org_id
                -- when p.custom_field_group_assignment_id = 19 then (select id from flow.list_of_value where parent_id = 353
                --                                                                                        and name = d2.site_survey_type)
                 else null end,
            case when p.custom_field_group_assignment_id = 11530 then d2.site_survey_scheduled_date else null end,
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1546 then ((site_survey_uploaded_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
(now() + interval '1 day')
     from flow.project p
              inner join blueraven.deal d on d.id = p.id
     where originator_id = 11 and (d.financier IS NULL OR (d.financier != '["One Roof Energy"]' and d.financier != '["Dividend Solar"]'))
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1728 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12882 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1689 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2027 then (select id from flow.list_of_value where parent_id = 1968
                                                                                                        and name = d2.price_change)
                 when p.custom_field_group_assignment_id = 2014 then (select id from flow.list_of_value where parent_id = 1799
            ...
            case when p.custom_field_group_assignment_id = 1936 then (select array_agg(id)
                                                                    from flow.list_of_value
                                                                    where parent_id = 2085 and
                                                                            name in (
                                                                            select  * from
            ...
            case when p.custom_field_group_assignment_id = 1741 then ((final_design_created_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 13181 then d2.site_survey_quality_issue
                 when p.custom_field_group_assignment_id = 8826 then  d2.pv_watts_estimate else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,date_value,
                                                         ...
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 1729 then ((site_survey_photos_missing_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC')
                 when p.custom_field_group_assignment_id = 12882 then site_survey_brs_no_show
                 when p.custom_field_group_assignment_id = 1689 then ((site_survey_verified_date AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 2027 then (select id from flow.list_of_value where parent_id = 1968
                                                                                                        and name = d2.price_change)
...
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
    ...
    (select p1.id,p.custom_field_group_assignment_id,
            case --when p.custom_field_group_assignment_id = 710 then credit_decision_date
                 when p.custom_field_group_assignment_id = 9541 then financial_agreement_sent_date else null end,
            case when p.custom_field_group_assignment_id = 1598 then ((final_design_sent_to_customer_date  AT TIME ZONE 'US/Mountain') AT TIME ZONE 'UTC') else null end,
            case when p.custom_field_group_assignment_id = 10009 then d2.system_size
                 when p.custom_field_group_assignment_id = 10035 then d2.referral_promotion_amount
                 when p.custom_field_group_assignment_id = 10048 then d2.total_ancillary_cost_with_fees
                 when p.custom_field_group_assignment_id = 10113 then d2.total_system_price
                 when p.custom_field_group_assignment_id = 10100 then d2.total_promotion_amount
                 when p.custom_field_group_assignment_id = 12362 then d2.loan_amount
                 when p.custom_field_group_assignment_id = 12349 then d2.total_cash_down_payment
                 else null end,
            case when p.custom_field_group_assignment_id = 10126 then (select id from flow.list_of_value where parent_id = 1981
                                                                                                        and name = d2.base_product)
                 when p.custom_field_group_assignment_id =11400 then (select plh.id from brs.proposal_log_history plh
                                                                                             inner join blueraven.deal d3 on plh.proposal_nbr::integer = d3.proposal_nbr
                                                                                             where d2.id = d3.id and plh.project_id =d2.id limit 1)
                 when p.custom_field_group_assignment_id =10815 then (select id from flow.list_of_value where parent_id = 1786
                                                                                                        and name = d2.inverter_brand)
                 when p.custom_field_group_assignment_id =9827 then d2.number_of_promotion_payments
                 when p.custom_field_group_assignment_id =10022 then d2.first_year_production_estimate
                 when p.custom_field_group_assignment_id =12336 then (select id from flow.list_of_value where parent_id = 2072
                                                                                                        and name = d2.secondary_financier)
                 when p.custom_field_group_assignment_id =9996 then panel_watts
                 when p.custom_field_group_assignment_id =9983 then panel_quantity
                 when p.custom_field_group_assignment_id =12375 then (select id from flow.list_of_value where parent_id = 2189
                                                                                                        and name::integer = d2.loan_term)
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9968 then (select id from flow.list_of_value where parent_id = 1864
                 and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12466 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12687 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10139 then (select id from flow.list_of_value where parent_id = 2254
                                                                                                        and name = d2.financier :: JSON #>> '{0}')
                 when p.custom_field_group_assignment_id =17272 then (select design_nbr
...
--                  when p.custom_field_group_assignment_id =709 then (select id from flow.list_of_value where parent_id = 81
--                                                                                                         and name = d2.credit_check)
                 when p.custom_field_group_assignment_id =9970 then (select id from flow.list_of_value where parent_id = 1864
                                                                                                        and name = d2.panel_brand)
                 when p.custom_field_group_assignment_id =12466 then number_of_arrays::integer
                 when p.custom_field_group_assignment_id =12687 then max_pitch::integer
                 when p.custom_field_group_assignment_id =10139 then (select id from flow.list_of_value where parent_id = 2254
...
(now() + interval '1 day')
     ...
     p as (
         ...
         from flow.custom_field_group_assignment cfga
                  ...
                  inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                  inner join flow.data_type dt on dt.id = cdt.data_type_id
         where cfg.process_step_id =1105
           and cf.archived is false and cfg.archived is false and cfga.archived is false
     )
insert into flow.project_process_step_custom_field_value(project_process_step_id, custom_field_group_assignment_id,
                                                         date_value,
                                                         date_created, date_modified, created_by_id, modified_by_id)
    (select p1.id,p.custom_field_group_assignment_id,
            case when p.custom_field_group_assignment_id = 6434 then final_design_signed_date
                 when p.custom_field_group_assignment_id = 15378 then agreement_signed_date
                 when p.custom_field_group_assignment_id = 16704 then countersign_date else null end,
            now(),now(),2350555,2350555
     from blueraven.deal d2
              inner join active_step p1 on p1.project_id = d2.id
              cross join p
