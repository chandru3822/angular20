...
insert into flow.project_custom_field_value (project_id, date_value, custom_field_group_assignment_id, timestamp_value, boolean_value, text_value, rich_text_value, numeric_value, int_value, int_array_value, created_by_id, date_created, modified_by_id, date_modified)
    select :sourceId, :dateValue::date,
           (select cfga.id from flow.custom_field_group_assignment cfga
                                  ...
                                  inner join flow.company_object_type cot on cfg.company_object_type_id = cot.id and cot.object_type_id = 1
    ...
    ON CONFLICT (project_id, custom_field_group_assignment_id)
      do update
...
select c.company_id
  ...
  //language=PostgreSQL
  public final static String updateValueUsingCfId = """
insert into flow.project_custom_field_value (project_id, date_value, custom_field_group_assignment_id, timestamp_value, int_value, created_by_id, date_created, modified_by_id, date_modified)
        values (:projectId, :dateValue::date, :customFieldGroupAssignmentId, :timestampValue::timestamp, :intValue, :userId, now(), :userId, now())
        ON CONFLICT (project_id, custom_field_group_assignment_id)
        do update
          set date_value = :dateValue::date,
          timestamp_value = :timestampValue::timestamp,
...
select
      ...
      coalesce((
                 ...
                 FROM (
                        ...
                        from flow.custom_field_group_assignment cfga
                               ...
                               inner join flow.custom_field cf on cf.id = cfga.custom_field_id or cf.id = cfga1.custom_field_id
                               inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                               left join flow.project_custom_field_value pcv on pcv.custom_field_group_assignment_id = cfga1.id and pcv.project_id = :idToUse
                               left join flow.project p on p.id = :idToUse
    ...
    from flow.custom_field_group cfg
        inner join flow.project_attachment_type pat on cfg.project_attachment_type_id = pat.id
