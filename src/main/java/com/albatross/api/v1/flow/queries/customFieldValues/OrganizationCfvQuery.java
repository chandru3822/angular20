...
public class OrganizationCfvQuery {
    ...
    select cfg.id,
           ...
           coalesce((
                        ...
                        FROM (
                                 select ocv.id,
                                        ocv.org_id as "orgId",
                                        ocv.date_value as "dateValue",
                                        ocv.timestamp_value as "timestampValue",
                                        ocv.boolean_value as "booleanValue",
                                        ocv.text_value as "textValue",
                                        ocv.rich_text_value as "richTextValue",
                                        ocv.numeric_value as "numericValue",
                                        ocv.int_value as "intValue",
                                        ocv.int_array_value as "intArrayValue",
                                ...
                                cf.custom_field_sql_smartlist as "customFieldSqlSmartlist",
                                                     ...
                                                     FROM (
                                                            ...
                                                            where lov.parent_id is not null
                                                              ...
                                                              and (lov.archived is not true OR
                                                                   (lov.archived is true AND (lov.id = ocv.int_value
                                                                     OR lov.id = any(ocv.int_array_value))))
                                                            order by
                                 ...
                                 from flow.custom_field_group_assignment cfga
                                          ...
                                          left join flow.organization_custom_field_value ocv on ocv.custom_field_group_assignment_id = cfga.id and ocv.org_id = :sourceId
  ...
  public final static String upsertCustomFieldValue = """
    insert into flow.organization_custom_field_value(org_id, date_value, custom_field_group_assignment_id, timestamp_value, boolean_value, text_value, rich_text_value, numeric_value, int_value, int_array_value, created_by_id, date_created, modified_by_id, date_modified)
        select :sourceId, :dateValue::date,
               (select cfga.id from flow.custom_field_group_assignment cfga
                                      ...
                                      inner join flow.company_object_type cot on cfg.company_object_type_id = cot.id and cot.object_type_id = 5
        ...
        ON CONFLICT (org_id, custom_field_group_assignment_id)
          DO UPDATE
          ...
          coalesce((
                     ...
                     FROM (
                            ...
                            from flow.custom_field_group_assignment cfga
                                   ...
                                   inner join flow.company_data_type cdt on cdt.id = cf.company_data_type_id
                                   left join flow.organization_custom_field_value pcv on pcv.custom_field_group_assignment_id = cfga1.id and pcv.org_id = :idToUse
                            where cfga.custom_field_group_id = cfg.id
                              and cfga.archived is not true
                            order by cfga.field_order, cf.field_name
                          ) fields), '[]') AS "customFieldValues"
        from flow.custom_field_group cfg
               inner join flow.org_attachment_type pat on cfg.org_attachment_type_id = pat.id
