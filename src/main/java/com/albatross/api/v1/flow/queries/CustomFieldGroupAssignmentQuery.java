...
public class CustomFieldGroupAssignmentQuery {
  ...
  public final static String getOne = """
    select
		cfg.id,
		cfg.group_name,
		cfg.company_object_type_id,
		cfg.group_order,
		cfg.archived,
		cfg.process_step_id,
		cfg.date_created, cfg.date_modified,
		cfg.created_by_id,
		cfg.modified_by_id,
		cfg.company_object_type_tab_id,
		...
		cfg.user_attachment_type_id,
		cfg.org_attachment_type_id,
		cfg.contact_attachment_type_id,
		cfg.project_attachment_type_id,
		coalesce(to_jsonb(cfg.company_process_step_status_type_ids), '[]') as "companyProcessStepStatusTypeIds",
		coalesce(to_jsonb(cfg.company_event_status_type_ids), '[]') as "companyEventStatusTypeIds",
		coalesce(to_jsonb(cfg.process_step_status_type_ids), '[]') as "processStepStatusTypeIds",
		coalesce(to_jsonb(cfg.event_status_type_ids), '[]') as "eventStatusTypeIds",
		(select coalesce(array_to_json(array_agg(occfg.object_category_id)), '[]') from flow.object_category_custom_field_group occfg where occfg.custom_field_group_id = cfg.id and occfg.archived is false) as "objectCategoryIds",
		cfg.ps_collapse_by_default,
		cfg.event_collapse_by_default,
          ot.object_type,
          cott.tab_name,
  ...
  public final static String getNonEventCustomFieldGroupsByProcessStep = """
    select cfg.id,
         cfg.group_name
      from flow.custom_field_group cfg
      where cfg.process_step_id = :processStepId
  ...
  public final static String getAvailableNativeFieldsForAttachmentType = """
    select cf.id, cf.field_name
       ...
       from flow.custom_field_group_assignment cfga
              inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
              inner join flow.custom_field cf on cf.id = cfga.custom_field_id
       where cfg.attachment_type_id = :attachmentTypeId
         and cfga.archived is not true
         and cfg.archived is not true
         and cf.archived is not true
       order by field_name
  ...
  public final static String moveFieldToOtherGroup = """
    update flow.custom_field_group_assignment
            set custom_field_group_id = :newGroupId,
...
""";
  ...
  public final static String insertWhiteListPosition = """
    insert into flow.white_listed_position(position_id, custom_field_group_assignment_id, white_list_type_id, company_id, created_by_id, date_created, modified_by_id, date_modified)
       select :positionId, :cfgaId, :whiteListTypeId, :companyId, :userId, now(), :userId, now()
  ...
  //language=PostgreSQL
  public final static String getIdsByPPSId = """
    select cfga.id
      from flow.custom_field_group_assignment cfga
      inner join flow.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
      inner join flow.custom_field cf on cf.id = cfga.custom_field_id
      inner join flow.process_step ps on ps.id = cfg.process_step_id
      inner join flow.project_process_step pps on pps.process_step_id = ps.id and pps.id = :ppsId
      where pps.archived is not true and
