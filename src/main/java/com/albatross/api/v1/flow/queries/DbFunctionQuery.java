...
public class DbFunctionQuery {
  ...
  //language=PostgreSQL
  public final static String getAvailableCompanies = """
    select *
    from flow.company c
    where c.parent_company_id is not null
    and c.archived is false
    and not exists (
      select cf.id from flow.company_function cf where db_function_id = :dbFunctionId and cf.company_id = c.id and cf.archived is false
    )
    order by c.parent_company_id, c.company_name
    """;
  ...
  //language=PostgreSQL
  public final static String getParameterTypes = """
     select pt.*
  ...
  public final static String getOne = """
    select df.*,
                ...
                coalesce((
                        SELECT array_to_json(array_agg(row_to_json(companyFunctions)))
                        FROM (
                                 select cf.id,
                                 ...
                                 where cf.db_function_id = df.id
                                 and cf.archived is not true
                             ) companyFunctions), '[]') AS "companyFunctions"
          from flow.db_function df
  ...
  public final static String addToCompany = """
     ...
     values (:displayName, :functionId, :companyId)
    """;

  //language=PostgreSQL
  public final static String getSystemValues = """
    select *
