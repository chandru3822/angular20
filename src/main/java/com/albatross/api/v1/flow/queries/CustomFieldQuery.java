...
public class CustomFieldQuery {
  ...
  public final static String getByDataView = """
    select  dvfc.id,
            ...
            null::int as data_view_child_field_config_id,
            dv.company_id,
            dvfc.display_name as field_name,
            cf.company_data_type_id, --note we will not always have this
            dvfc.date_created,
            dvfc.date_modified,
            dvfc.created_by_id,
            dvfc.modified_by_id,
            dvfc.archived,
            case when df.object_type_id = 6 and df.column_name = 'resource_id' then 6
                 else coalesce(df.data_type_id, cdt.data_type_id) end as data_type_id
    ...
    select  dvcfc.id,
            dvfc.id as data_view_field_config_id,
            dvcfc.id as data_view_child_field_config_id,
            dv.company_id,
            dvcfc.display_name as field_name,
            null::int as company_data_type_id,
            dvcfc.date_created,
            dvcfc.date_modified,
            dvcfc.created_by_id,
            dvcfc.modified_by_id,
            dvcfc.archived,
