...
public class HubspotWebhookController {
    ...
    public void saveContact(@RequestBody HubspotContact contact) throws Exception {
        if (ricochetEnabled) {
            ...
            address.setAddress1(contact.getProperties().getAddress().getValue());
            address.setCity(contact.getProperties().getCity().getValue());
            address.setState(contact.getProperties().getState().getValue());
