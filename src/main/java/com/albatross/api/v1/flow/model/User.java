...
public class User {
  ...
  // probably should have had the company specific stuff in a company request instead of the user
  // request.
  private Long id,
      companyId,
      minuteIncrement,
      ...
      homePageCompanyFeatureId,
      masqueradingUserId;
  private int loginAttempts;
  // not sure if i should put this here.  there are times when i need a unique list of users so i
  // can coalesce their positions. but other times i need lists of all users so 1 user can show up
  ...
  // highestCompanyId = highest company a user has access to (regardless of context and hierarchies)
  // <- mostly used to determine if user is system admin/has access to albatross
  ...
  public boolean isUnlocked() {
    return this.getHighestCompanyId() == 1L
        || (null == this.getHasAccess() ? false : this.getHasAccess());
  }
