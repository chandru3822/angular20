...
public class ExpenseBudgetService {
  ...
  public List<BudgetTemplate> getAllBudgetTemplates() {
    return sqlCache.query("expenseBudget.templates.getAll", Collections.emptyMap(), BudgetTemplate.class);
  }
  ...
  public Optional<BudgetTemplate> updateBudgetTemplate(BudgetTemplate budgetTemplate){
    ...
    if(null != budgetTemplate.getId()){
      ...
      Long id = sqlCache.updateReturningId("expenseBudget.templates.insertBudgetTemplate", params, "id").longValue();
      templateId = id;
      ...
      //creates an expense budget if a budget does not already exist for this user, budget type, and month
      Optional<ExpenseBudget> expenseBudget = sqlCache.get("expenseBudget.checkIfExistsByTemplate", templateParams, ExpenseBudget.class);

      if(!expenseBudget.isPresent()){

        params.put("startDate", firstDayOfMonth);
        params.put("endDate", lastDayOfMonth);

        Long newId = sqlCache.updateReturningId("expenseBudget.insertOriginalExpenseBudget", params, "id").longValue();
