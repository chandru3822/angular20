...
public class ContactLeadService {
  ...
  public void saveContactLead(ContactLead cl) {
    ...
    String stateValue =
        state == null
            ? null
            : State.valueOfName(state) != State.UNKNOWN
                ? State.valueOfName(state).toString()
                : State.valueOfAbbreviation(state.toUpperCase()) != State.UNKNOWN
                    ? State.valueOfAbbreviation(state.toUpperCase()).toString()
                    : null;

    Double latitude = null, longitude = null;
    // even if the state value is null, try to get a valid lat/long if there is at least an address
    // and a city
    ...
    if (cl.getLeadSourceDetail() != null) {
      String leadSourceDetailId =
          checkIfCustomFieldDropdownValueExists(543, cl.getLeadSourceDetail());
    ...
    if (cl.getReferralGenerationRepresentative() != null) {
      ...
      List<ListOfValue> listOfValues =
          systemListService.getSystemListOptionsForCompany(
              2L, true, Arrays.asList(494L, 76L), null, 3L);
      ListOfValue selectedValue =
          listOfValues.stream()
              .filter(l -> l.getName().equals(cl.getReferralGenerationRepresentative()))
              .findFirst()
              .orElse(null);
...
//      log.error(msg, e.getMessage());
  ...
  public void processHubspotCustomFieldValues(
      HubspotLead lead, Long contactId, Long leadOwnerUserId) {
    ArrayList<CustomFieldValue> cfvList = new ArrayList<>();
    HashMap<String, Object> params = new HashMap<>();
    params.put("contactId", contactId);
    params.put("leadOwnerUserId", leadOwnerUserId);

    // if "null" is returned for leadStatusId, then we don't want to save it, b/c that means it's
    // not one of the existing options, and we don't save new values
    ...
    if (!lead.getLead_source_detail().isBlank()) {
      String leadSourceDetailId =
          checkIfCustomFieldDropdownValueExists(543, lead.getLead_source_detail());
      CustomFieldValue leadSourceDetail = new CustomFieldValue();
      leadSourceDetail.setFieldName("Lead Source Detail");
