...
public class ProposalQuery {
  ...
  public final static String postalCodeApproved = """
    ...
    with published_proposal as (select proposal_version_id from brs.primary_company_proposal_version where company_id = 3),
         ...
         approved_postal_code as (select p as postal_code
                                  ...
                                  where field_id = 122 -- Postal Code
                                    and p = (select postal_code from flow.project p2 where p2.id = :projectId)),
         requested_process_step as (select pps.id,
                                    ...
                                    from flow.project_process_step pps
                                             ...
                                             left join flow.project_process_step_custom_field_value ppscfv1
                                                       on ppscfv1.project_process_step_id = pps.id
                                                           and ppscfv1.custom_field_group_assignment_id =
                                                               23623 -- postal code comments
                                    ...
                                    where pps.project_id = :projectId
                                      ...
                                      and cpsst.process_step_status_type_id in (1, 2))
    select rps.id,
           rps.process_step_status_type_id,
           rps.process_step_status_type,
           rps.comments,
           case when t.approved then t.approved else coalesce(rps.process_step_approval, false) end as approved
