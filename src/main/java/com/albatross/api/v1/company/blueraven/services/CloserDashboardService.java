...
public class CloserDashboardService {
  ...
  private JSONArray processCloserData(Integer timeInterval, boolean officeFdcRank, Long selectedOrgId) {
    ...
    String sql = officeFdcRank ? "closerDashboard.getCloserTableScoresOffice" : "closerDashboard.getCloserTableScoresRep";

    List<CloserTableScores> closerTableScores = sqlCache.query(sql, params, CloserTableScores.class);
  ...
  public String funnelDrilldownStandard(FunnelRequest funnelRequest) {
    String sqlQuery = "select brs.rpt_closer_funnel_standard_and_cohort_drilldown_fancy(:startDate::date, :endDate::date, :funnelId::integer, array[ :userIds ]::integer[], array[ :orgIds ]::integer[], :isCheckedInColumn::boolean)";
