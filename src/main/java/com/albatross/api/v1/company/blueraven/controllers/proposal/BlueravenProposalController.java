...
public class BlueravenProposalController {
  ...
  @PreAuthorize("hasFeatureAccessLevel('PROPOSALS_ADMIN', 'PROPOSALS_MANAGE') || isBrSystemUser()")
  public void updateProposalDetails(@PathVariable Long proposalId,
                                    @RequestBody Proposal proposal,
                                    @AuthenticationPrincipal UserAccountDetails details) {
    proposalService.updateProposalDetails(proposalId, proposal, details);
  }
  ...
  public ProposalFilterResponse getFilterableOptions(@PathVariable Long proposalId,
    ...
    final Proposal proposal = proposalService.getSimpleProposal(proposalId).orElseThrow(NotFoundException::new);
    final List<Long> filterIds = proposalVersionService.getProposalValuesFilterIds(proposal, filter);
