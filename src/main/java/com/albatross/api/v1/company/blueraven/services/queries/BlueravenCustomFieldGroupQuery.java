...
public class BlueravenCustomFieldGroupQuery {
           ...
           coalesce((SELECT array_to_json(array_agg(row_to_json(customFields)))
                     FROM (SELECT cfga.id,
                           ...
                           FROM brs.custom_field_group_assignment cfga
                                    ...
                                    inner join brs.custom_field_group cfg3 on cfg3.id = cfga.custom_field_group_id
                                    inner join flow.company_data_type cdt on cf.company_data_type_id = cdt.id
                                    inner join brs.object_type ot on cfg3.object_type_id = ot.id
                           WHERE cfga.custom_field_group_id = cfg.id
                             AND cfga.archived is not true
                             and cfg.object_type_id = :objectTypeId
                           union all
                           SELECT cfga.id,
                                  cfga.custom_field_group_id                      as "customFieldGroupId",
                                  cfga.ancillary_custom_field_group_assignment_id as "ancillaryCustomFieldGroupAssignmentId",
                                  ...
                                  ot2.object_type                                 as "objectType",
                                  '[]'                                            as whiteListedPositions,
                                  '[]'                                            as hiddenWhiteListedPositions
                           FROM brs.custom_field_group_assignment cfga
                                    inner join flow.custom_field_group_assignment cfga2
                                               on cfga2.id = cfga.ancillary_custom_field_group_assignment_id
                                    inner join flow.custom_field cf2 on cf2.id = cfga2.custom_field_id
                                    inner join flow.company_data_type cdt on cf2.company_data_type_id = cdt.id
                                    inner join flow.custom_field_group cfg2 on cfg2.id = cfga2.custom_field_group_id
                                    inner join flow.company_object_type cot2 on cfg2.company_object_type_id = cot2.id
                                    inner join flow.object_type ot2 on cot2.object_type_id = ot2.id
                                    left join flow.process_step ps on cfg2.process_step_id = ps.id
                           WHERE cfga.custom_field_group_id = cfg.id
                             AND cfga.archived is not true
                             AND cfga2.archived is not true
  ...
  public final static String insertCustomFieldGroup = """
    insert into brs.custom_field_group(group_name, object_type_id, group_order, created_by_id, date_created, modified_by_id, date_modified, column_number)
    values (:groupName, :objectTypeId, (select coalesce(max(group_order) + 1, 0) from brs.custom_field_group where object_type_id = :objectTypeId and column_number = :columnNumber and archived is false), :createdById, now(), :createdById, now(), :columnNumber)
    """;

  //language=PostgreSQL
  public final static String moveGroupToColumn = """
    update brs.custom_field_group
    set column_number = :columnNumber,
        modified_by_id = :userId,
        date_modified = now(),
        group_order = (select coalesce(max(group_order) + 1, 0) from brs.custom_field_group where object_type_id = :objectTypeId and column_number = :columnNumber and archived is false)
    where id = :id
    """;
    ...
    select cfg.*,
           ...
           coalesce((
                      ...
                      FROM (
                             ...
                             FROM brs.custom_field_group_assignment cfga
                                    inner join brs.custom_field cf on cf.id = cfga.custom_field_id
                             WHERE cfga.custom_field_group_id = cfg.id AND cfga.archived is not true) customFields), '[]') AS "customFields"
    from brs.custom_field_group cfg
  ...
  public final static String getAvailableCustomFieldsInGroup = """
    ...
    select cf.id,cf.field_name
    from brs.custom_field_group_assignment cfga
           inner join brs.custom_field_group cfg on cfg.id = cfga.custom_field_group_id
           inner join brs.custom_field cf on cf.id = cfga.custom_field_id
           inner join brs.object_type cot on cot.id = cfg.object_type_id
  ...
  public final static String addFieldToGroup = """
    insert into brs.custom_field_group_assignment(custom_field_group_id, custom_field_id, ancillary_custom_field_group_assignment_id, field_order, created_by_id, date_created, modified_by_id, date_modified)
    values (:customFieldGroupId, :customFieldId, :ancillaryCustomFieldGroupAssignmentId, (select coalesce(max(field_order) + 1, 0) from brs.custom_field_group_assignment where custom_field_group_id = :customFieldGroupId and archived is not true), :createdById, now(), :createdById, now())
    """;
  ...
  //language=PostgreSQL
  public final static String saveRequired = """
    update brs.custom_field_group_assignment
  ...
  //language=PostgreSQL
  public final static String saveMinMax = """
    update brs.custom_field_group_assignment
  ...
  //language=PostgreSQL
  public final static String updateConditionalOnId = """
    update brs.custom_field_group_assignment
    set conditional_on_cfga_id = :conditionalOnId,
        modified_by_id         = :modifiedById,
        date_modified          = now()
    where id = :cfgaId
    """;
  ...
  //language=PostgreSQL
  public final static String updateReadOnly = """
    update brs.custom_field_group_assignment
    set read_only              = :value,
        modified_by_id         = :modifiedById,
        date_modified          = now()
  ...
  //language=PostgreSQL
  public final static String updateHidden = """
    update brs.custom_field_group_assignment
    set hidden                 = :value,
        modified_by_id         = :modifiedById,
        date_modified          = now()
  ...
  //language=PostgreSQL
  public final static String insertWhiteList = """
    insert into brs.white_listed_position ( custom_field_group_assignment_id, position_id, created_by_id, modified_by_id, white_list_type_id)
        values (?, ?, ?, ?, ?)
        on conflict (custom_field_group_assignment_id, position_id, white_list_type_id)
            where white_listed_position.archived is false
            do update
  ...
  public final static String archiveWhiteListPositionsNoLongerUsed = """
    update brs.white_listed_position
      set archived = true,
          date_modified = now(),
          modified_by_id = :userId
    where custom_field_group_assignment_id = :cfgaId
      and position_id not in (:positionIds)
