    ...
    smartlistIds.forEach(id -> {
      try {
        ...
        if (!currentlyFailingProjectDetails.contains(id)) {
          log.error("Failed on smartlist ID: %s".formatted(id));
  ...
  void processStepMain() {
    ...
    smartlistIds.forEach(id -> {
      try {
        ...
        if (!isValid) {
          throw new RuntimeException("Invalid Syntax, Smartlist ID: %s, Query: %s".formatted(id, sql));
        }
      } catch (Exception e) {
